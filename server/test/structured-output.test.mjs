...
test('yunwu.chat：responseFormat 随请求体下发 response_format 字段', async () => {
  ...
  globalThis.fetch = async (url, opts) => {
    ...
    return {
      ok: true,
      json: async () => ({ choices: [{ message: { content: '{"title":"t","subtitle":"s","pages":[]}' } }], usage: { prompt_tokens: 10, completion_tokens: 5 } }),
    };
  };
  try {
...
test('generate：responseSchema 经云雾通道下发，正文即合法 JSON（不再靠正则抠）', async () => {
  ...
  globalThis.fetch = async (url, opts) => {
    ...
    return {
      ok: true,
      json: async () => ({ choices: [{ message: { content: '{"theme":"品鉴会"}' } }], usage: { prompt_tokens: 8, completion_tokens: 4 } }),
    };
  };
  try {
