...
test('生产装配按 auth→tenantScope→tenantGate→moduleGuard→aiGuard→Router 顺序拒绝并隔离', async () => {
  ...
  assert.equal(ownContents.payload.total, 1);
  assert.deepEqual(ownContents.payload.rows.map(item => item.title), ['本企业内容']);
  assert.doesNotMatch(JSON.stringify(ownContents.payload), /其他企业内容|不得向企业101泄露/u);
