...
test("门店成本与利润数据仅管理角色可读，一线员工即使直连路由也被拒绝", async () => {
  await withServer(salesOne, async (base) => {
    const { response, body } = await request(
      base,
      "/store-data/kpi?month=2026-07",
    );
    assert.equal(response.status, 403);
    assert.match(body.error, /无权限/);
  });
});
...
test("门店 CRUD：创建/更新/校验/删除守卫", async () => {
  await withServer(userOne, async (base) => {
    const bad = await request(base, "/store-data/stores", "POST", {
      name: "",
      biz_type: "正餐",
    });
    ...
    const badType = await request(base, "/store-data/stores", "POST", {
      name: "类型错误店",
      biz_type: "烧烤",
    });
    ...
    const updated = await request(
      base,
      `/store-data/stores/${storeOneId}`,
      "PUT",
      { status: "筹备中", address: "长风街1号" },
    );
    assert.equal(updated.response.status, 200);
    assert.equal(updated.body.status, "筹备中");
    assert.equal(updated.body.address, "长风街1号");
    await request(base, `/store-data/stores/${storeOneId}`, "PUT", {
      status: "营业中",
    });
...
test("菜品 CRUD：门店归属校验、筛选与上下架", async () => {
  await withServer(userOne, async (base) => {
    const orphan = await request(base, "/store-data/dishes", "POST", {
      store_id: 9999,
      name: "无主菜",
    });
    ...
    const badPrice = await request(base, "/store-data/dishes", "POST", {
      store_id: storeOneId,
      name: "负价菜",
      price: -1,
    });
    ...
    const off = await request(
      base,
      `/store-data/dishes/${dishBId}/status`,
      "POST",
      { status: "下架" },
    );
    ...
    const filtered = await request(
      base,
      `/store-data/dishes?store_id=${storeOneId}&status=下架`,
    );
    assert.equal(filtered.body.total, 1);
    assert.equal(filtered.body.rows[0].name, "柠檬茶");
    assert.equal(filtered.body.rows[0].store_name, "长风街旗舰店");
    await request(base, `/store-data/dishes/${dishBId}/status`, "POST", {
      status: "在售",
    });

    const byCategory = await request(
      base,
      "/store-data/dishes?category=招牌菜",
    );
...
test("成本 CRUD：类别/日期/金额校验与按月按店筛选", async () => {
  await withServer(userOne, async (base) => {
    const badCat = await request(base, "/store-data/costs", "POST", {
      store_id: storeOneId,
      date: "2026-07-05",
      category: "装修",
      amount: 100,
    ...
    const badAmount = await request(base, "/store-data/costs", "POST", {
      store_id: storeOneId,
      date: "2026-07-05",
      category: "食材",
      amount: 0,
    ...
    const c1 = await request(base, "/store-data/costs", "POST", {
      store_id: storeOneId,
      date: "2026-07-05",
      category: "食材",
      amount: 60,
    ...
    const c2 = await request(base, "/store-data/costs", "POST", {
      store_id: storeOneId,
      date: "2026-07-12",
      category: "人力",
      amount: 40,
    ...
    const other = await request(base, "/store-data/costs", "POST", {
      store_id: storeOneId,
      date: "2026-06-20",
      category: "房租",
      amount: 999,
    ...
    const del = await request(
      base,
      `/store-data/costs/${other.body.id}`,
      "DELETE",
    );
...
test("订单明细：校验订单归属、批量登记与读取", async () => {
  ...
  await withServer(userOne, async (base) => {
    const missing = await request(
      base,
      "/store-data/orders/999999/items",
      "POST",
      { items: [{ dish_id: dishAId, qty: 1 }] },
    );
    assert.equal(missing.response.status, 404);
    assert.match(missing.body.error, /订单不存在/);

    const badQty = await request(
      base,
      `/store-data/orders/${orderOne}/items`,
      "POST",
      {
    ...
    const second = await request(
      base,
      `/store-data/orders/${orderTwo}/items`,
      "POST",
      {
    ...
    assert.equal(
      q.get("SELECT store_id FROM orders WHERE tenant_id=1 AND id=?", orderOne)
        .store_id,
      storeOneId,
    );
    ...
    try {
      const failed = await request(
        base,
        `/store-data/orders/${rollbackOrder}/items`,
        "POST",
        {
...
test("KPI 计算：真实客单价 / 菜品TOP / 毛利率 / 盈亏平衡与月营收成本", async () => {
  await withServer(userOne, async (base) => {
    ...
    const scoped = await request(
      base,
      `/store-data/kpi?month=2026-07&store_id=${storeOneId}`,
    );
...
test("跨租户隔离：租户二读不到租户一的门店/菜品/成本/订单明细", async () => {
  await withServer(userTwo, async (base) => {
    ...
    const dishHit = await request(
      base,
      `/store-data/dishes/${dishAId}/status`,
      "POST",
      { status: "下架" },
    );
    ...
    // 租户二给租户一的订单挂明细必须被拒绝
    const orderOneId = q.get(
      `SELECT id FROM orders WHERE tenant_id=1 ORDER BY id LIMIT 1`,
    )?.id;
    const cross = await request(
      base,
      `/store-data/orders/${orderOneId}/items`,
      "POST",
      {
    ...
    // 建自己的门店后互不可见
    const own = await request(base, "/store-data/stores", "POST", {
      name: "租户二门店",
      biz_type: "茶饮",
    });
    ...
    assert.equal(
      db.prepare("SELECT tenant_id FROM stores WHERE id=?").get(storeTwoId)
        .tenant_id,
      2,
    );
  });
  await withServer(userOne, async (base) => {
    const list = await request(base, "/store-data/stores");
    assert.ok(
      !list.body.rows.some((s) => s.id === storeTwoId),
      "租户一不应看到租户二门店",
    );
    // 租户一菜品仍是在售（租户二越权下架被拒绝）
    assert.equal(
      db.prepare("SELECT status FROM dishes WHERE id=?").get(dishAId).status,
      "在售",
    );
  });
});
...
test("门店删除守卫：有关联菜品/成本时禁止删除，清理后可删", async () => {
  await withServer(userOne, async (base) => {
    ...
    // 空门店可直接删除
    const empty = await request(base, "/store-data/stores", "POST", {
      name: "待关空店",
      biz_type: "快餐",
    });
    const removed = await request(
      base,
      `/store-data/stores/${empty.body.id}`,
      "DELETE",
    );
