...
function tenantDisplayName(tid = curTenant()) {
  const name = String(
    q.get("SELECT name FROM tenants WHERE id=?", Number(tid))?.name || "",
  )
    .replace(/\s+/g, " ")
    .trim();
...
function roleLabel(role) {
  return (
    { boss: "老板", ops_director: "运营总监", admin: "管理员" }[role] ||
    role ||
    "管理者"
  );
}
...
function dedupeRecipients(list = []) {
  ...
  return list.filter((r) => {
    ...
    const receiveIdType = normalizeReceiveIdType(
      r.receiveIdType || r.calendarUserIdType || "open_id",
    );
...
function upsertManagerReceiver(cfg, user = {}, receiver = {}) {
  ...
  const receiveIdType = normalizeReceiveIdType(
    receiver.receiveIdType || "open_id",
  );
  const receiveId = String(receiver.receiveId || "").trim();
  if (!receiveId) return cfg;
  const managerReceivers = Array.isArray(cfg.managerReceivers)
    ? cfg.managerReceivers
    : [];
  const nextItem = {
    userId: user.id || null,
    ...
    calendarUserIdType: CALENDAR_USER_ID_TYPES.has(receiveIdType)
      ? receiveIdType
      : "open_id",
    boundAt: new Date().toISOString(),
  };
  const next = managerReceivers.filter((x) => {
    if (nextItem.userId && Number(x.userId) === Number(nextItem.userId))
      return false;
    return (
      `${x.receiveIdType}:${x.receiveId}` !==
      `${nextItem.receiveIdType}:${nextItem.receiveId}`
    );
  });
  next.push(nextItem);
...
export function feishuUserRecipient(userId, tid = curTenant()) {
  ...
  return (
    candidates.find(
      (r) => Number(r.userId) === Number(userId) && activeBoundUser(r, tid),
    ) || null
  );
}
...
export function feishuManagerRecipients(tid = curTenant()) {
  ...
  const managers = (
    ...
    activeBoundUser(recipient, tid, { managerOnly: true }),
  );
...
export function feishuManagerSummary(tid = curTenant()) {
  ...
  return {
    ...
    calendarCount: recipients.filter(
      (r) =>
        r.calendarUserId &&
        CALENDAR_USER_ID_TYPES.has(r.calendarUserIdType || r.receiveIdType),
    ).length,
...
export function feishuConfig(tid = curTenant()) {
  const cfg = tcfg("feishu", {}, tid) || {};
  // 部署级环境变量只兼容总部租户1。其他企业必须显式保存自己的应用凭据，
  // 不能拿总部 App Secret 去访问其自定义的多维表格或接收人。
  const allowLegacyDeploymentCredential = Number(tid) === 1;
  const receiveIdType = normalizeReceiveIdType(
    cfg.receiveIdType || cfg.receiverIdType || cfg.userIdType,
  );
  const receiveId =
    cfg.receiveId || cfg.openId || cfg.userId || cfg.unionId || cfg.email || "";
  const hasBoundRecipient = [
    ...(Array.isArray(cfg.userReceivers) ? cfg.userReceivers : []),
    ...(Array.isArray(cfg.managerReceivers) ? cfg.managerReceivers : []),
  ].some(
    (item) =>
      String(item?.receiveId || "").trim() && activeBoundUser(item, tid),
  );
  return {
    ...
    calendarUserId:
      cfg.calendarUserId ||
      (CALENDAR_USER_ID_TYPES.has(receiveIdType) ? receiveId : ""),
    calendarUserIdType:
      cfg.calendarUserIdType ||
      (CALENDAR_USER_ID_TYPES.has(receiveIdType) ? receiveIdType : "open_id"),
    calendarId: cfg.calendarId || "",
    bitableUrl: cfg.bitableUrl || "",
  };
}
export const appReady = (tid = curTenant()) => {
  const c = feishuConfig(tid);
  return !!(c.appId && c.appSecret);
};
export const appBotReady = (tid = curTenant()) => {
  const c = feishuConfig(tid);
...
export async function feishuTenantToken(
  tid = curTenant(),
  { fetchFn = fetch, useCache = fetchFn === fetch } = {},
) {
  const c = feishuConfig(tid);
  if (!c.appId || !c.appSecret)
    throw new Error("未配置飞书企业应用凭据（App ID/Secret）");
  const cached = useCache ? tokenCache.get(tid) : null;
  ...
  if (!d.tenant_access_token) throw new Error("飞书鉴权未返回租户访问凭证");
  if (useCache)
    tokenCache.set(tid, {
      token: d.tenant_access_token,
      exp: Date.now() + Math.max(60, Number(d.expire || 7200) - 300) * 1000,
    });
  return d.tenant_access_token;
}
const tenantToken = feishuTenantToken;
...
export async function createOAuthBinding({
  tid = curTenant(),
  user = {},
  baseUrl = "",
} = {}) {
  ...
  const c = feishuConfig(tid);
  if (!c.appId || !c.appSecret)
    throw new Error("后台未配置飞书企业应用 App ID / App Secret");
  const origin = String(baseUrl || "").replace(/\/+$/, "");
  const configuredRedirectUri = String(
    process.env.FEISHU_REDIRECT_URI || "",
  ).trim();
  if (!origin && !configuredRedirectUri)
    throw new Error("无法生成飞书授权回调地址");
  const redirectUri =
    configuredRedirectUri || `${origin}/api/public/feishu/oauth/callback`;
  ...
  return {
    ...
    localMode: /^https?:\/\/(localhost|127\.0\.0\.1|::1)(:|\/|$)/i.test(
      redirectUri,
    ),
  };
}
...
export function oauthBindingStatus(state, tid = curTenant(), userId = null) {
  ...
  if (
    !s ||
    s.tid !== tid ||
    (userId != null && Number(s.userId) !== Number(userId))
  )
    return { status: "missing" };
  if (s.exp < Date.now() && s.status === "pending")
...
export async function handleFeishuOAuthCallback({ code, state } = {}) {
  ...
  try {
    const userAccessToken = await exchangeUserAccessToken(
      String(code),
      s.redirectUri,
      s.tid,
    );
    ...
    if (!openId)
      throw new FeishuBindingError(
        "飞书未返回 open_id，请确认应用已开通网页应用免登/用户信息权限",
      );
    const cur = tcfg("feishu", {}, s.tid) || {};
    const receiverName =
      info.name || info.en_name || info.email || info.mobile || "飞书用户";
    const actor = s.userId
      ? q.get(
          "SELECT id,name,role,status FROM users WHERE tenant_id = ? AND id = ?",
          s.tid,
          s.userId,
        )
      : null;
...
async function sendAppMessage(
  ...
  const res = await fetch(
    `https://open.feishu.cn/open-apis/im/v1/messages?receive_id_type=${encodeURIComponent(type)}`,
    {
      method: "POST",
      headers: {
...
function buildCard({ title, lines = [], url }, tid = curTenant()) {
  ...
  return {
    header: {
      title: {
        tag: "plain_text",
        content: `Nano Work · ${tenantName}｜${notificationTitle(title)}`,
      },
      template: "blue",
    },
    elements: [
      {
        ...
        text: {
          tag: "lark_md",
          content: lines.map(notificationLine).join("\n"),
        },
      },
      ...(url
        ? [
            {
              ...
              actions: [
                {
                  tag: "button",
                  text: { tag: "plain_text", content: "打开 Nano Work" },
                  type: "primary",
                  url,
                },
              ],
...
export async function pushFeishu(msg, tid = curTenant()) {
  ...
  if (!c.appId || !c.appSecret)
    return { skipped: true, reason: "未配置飞书企业应用凭据" };
  if (!c.receiveId)
    return { skipped: true, reason: "未配置飞书应用机器人接收人" };
...
export async function pushFeishuToManagers(msg, tid = curTenant()) {
  ...
  if (!recipients.length)
    return { skipped: true, reason: "暂无已绑定的老板/管理层飞书接收人" };
  const card = buildCard(msg, tid);
  const results = [];
...
export async function pushFeishuToUser(msg, userId, tid = curTenant()) {
  ...
  try {
    const out = await sendAppMessage(
      recipient.receiveId,
...
export async function bindAppBotTarget(
  input = {},
  tid = curTenant(),
  actor = {},
) {
  ...
  const rawReceiveId =
    input.receiveId !== undefined && !String(input.receiveId).includes("****")
      ? input.receiveId
      : cur.receiveId;
  const receiveId = String(rawReceiveId ?? "").trim();
  const receiveIdType = normalizeReceiveIdType(
    input.receiveIdType ?? cur.receiveIdType,
  );
  ...
  const base = {
    ...
    appId:
      input.appId !== undefined && !String(input.appId).includes("****")
        ? String(input.appId).trim()
        : cur.appId || "",
    appSecret:
  ...
  if (
    !(next.appId || effective.appId) ||
    !(next.appSecret || effective.appSecret)
  )
    throw new Error("请先配置飞书企业应用 App ID / App Secret");
...
export function tenantByIcsToken(key) {
  if (!key) return null;
  const row = q.get(
    "SELECT key FROM sys_config WHERE key LIKE 'feishu_ics_token:%' AND value = ?",
    JSON.stringify(key),
  );
  if (!row) return null;
...
export function buildIcs(activities = [], explicitTid = null) {
  const inferredTid =
    Number(
      explicitTid ||
        activities.find((activity) => Number(activity?.tenant_id))?.tenant_id ||
        curTenant(),
    ) || 1;
  ...
  const lines = [
    "BEGIN:VCALENDAR",
    "VERSION:2.0",
    `PRODID:-//Nano Work//${productTenant} Activities//CN`,
    `X-WR-CALNAME:${esc(identity.summary)}`,
  ...
  for (const [index, a] of activities.entries()) {
    ...
    const stableId = String(a.id || `${d}-${index + 1}`).replace(
      /[^a-zA-Z0-9_.-]/g,
      "-",
    );
    lines.push(
...
async function ensureCalendar(tid = curTenant()) {
    ...
    if (Number(cur.calendarIdentityVersion || 0) < CALENDAR_IDENTITY_VERSION) {
      try {
        ...
        const response = await fetch(
          `https://open.feishu.cn/open-apis/calendar/v4/calendars/${cur.calendarId}`,
          {
            method: "PATCH",
            headers: {
              Authorization: `Bearer ${token}`,
        ...
        if (updated.code === 0)
          setTenantConfig(
            "feishu",
            { ...cur, calendarIdentityVersion: CALENDAR_IDENTITY_VERSION },
            tid,
          );
      } catch {
  ...
  setTenantConfig(
    "feishu",
    {
      ...cur,
      calendarId: id,
      calendarIdentityVersion: CALENDAR_IDENTITY_VERSION,
    },
    tid,
  );
  return id;
...
async function ensureEventAttendees(calendarId, eventId, tid = curTenant()) {
  const recipients = feishuManagerRecipients(tid).filter(
    (r) =>
      r.calendarUserId &&
      CALENDAR_USER_ID_TYPES.has(r.calendarUserIdType || r.receiveIdType),
  );
  if (!recipients.length)
    return { skipped: true, reason: "暂无可加入日历的老板/管理层飞书用户ID" };
  const token = await tenantToken(tid);
  ...
  for (const [userIdType, list] of groups) {
    const res = await fetch(
      `https://open.feishu.cn/open-apis/calendar/v4/calendars/${calendarId}/events/${eventId}/attendees?user_id_type=${encodeURIComponent(userIdType)}`,
      {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
        ...
        body: JSON.stringify({
          attendees: list.map((r) => ({
            type: "user",
            is_optional: false,
            user_id: r.calendarUserId || r.receiveId,
          })),
    ...
    results.push(
      d.code === 0
        ...
        : {
            ...
            userIdType,
            count: list.length,
            error: feishuResponseError(res.status, d).message,
          },
    );
  }
...
export async function syncActivityToFeishuCalendar(act, tid = curTenant()) {
  ...
  if (!act?.id || !act?.date)
    return { skipped: true, reason: "活动缺少ID或日期" };
  ...
  const toTs = (date, t) =>
    String(
      Math.floor(
        new Date(
          `${date}T${t.slice(1, 3)}:${t.slice(3, 5)}:00+08:00`,
        ).getTime() / 1000,
      ),
    );
  ...
  let d;
  let eventId = eventMap[act.id];
  if (eventMap[act.id]) {
    // 已有日程 → PATCH 更新
    const res = await fetch(
      `https://open.feishu.cn/open-apis/calendar/v4/calendars/${calId}/events/${eventMap[act.id]}`,
      {
        method: "PATCH",
        headers: {
          Authorization: `Bearer ${token}`,
    ...
    const res = await fetch(
      `https://open.feishu.cn/open-apis/calendar/v4/calendars/${calId}/events`,
      {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
  ...
  if (d.code !== 0)
    return {
      ok: false,
      error: feishuResponseError(502, d).message,
      attendee: null,
    };
  const attendee = eventId
    ? await ensureEventAttendees(calId, eventId, tid).catch((e) => ({
        ok: false,
        error: safeFeishuError(e).message,
      }))
    : null;
...
export async function syncActivityLifecycleToFeishu(
  act,
  { tid = curTenant(), actor = null, action = "updated", url = "" } = {},
) {
  const actionLabel =
    {
      created: "新活动创建",
      updated: "活动信息更新",
      status: `活动状态更新：${act?.status || ""}`,
      result: "活动战果更新",
      approved: "活动策划审批通过，已同步日历",
    }[action] || "活动信息更新";
  const calendar = await syncActivityToFeishuCalendar(act, tid).catch((e) => ({
    ok: false,
    error: safeFeishuError(e).message,
  }));
