...
function cleanBase64(raw = '') {
  return String(raw).replace(/^data:[^;]+;base64,/, '').replace(/\s/g, '');
}

export function decodeBase64File(raw, maxBytes = MAX_FILE_BYTES) {
  const encoded = cleanBase64(raw);
  if (encoded.length > Math.ceil(maxBytes * 4 / 3) + 8 || !/^[A-Za-z0-9+/]*={0,2}$/.test(encoded)) {
    throw Object.assign(new Error('文件编码无效或超过大小上限'), { status: 400 });
  }
  const buffer = Buffer.from(encoded, 'base64');
  if (!buffer.length) throw Object.assign(new Error('文件内容为空'), { status: 400 });
  if (buffer.length > maxBytes) throw Object.assign(new Error('文件超过大小上限'), { status: 400 });
  return buffer;
}

function safePart(value, fallback = 'file') {
  const out = String(value || '').replace(/[^\w.\-\u4e00-\u9fa5]/g, '_').replace(/_+/g, '_').slice(0, 120);
  return out || fallback;
}
...
function publicFile(row) {
  ...
  return { ...safe, preview: String(extracted_text || '').slice(0, 1800), readable: !!String(extracted_text || '').trim() };
}

export function saveUploadedFile({ name, b64, mime = '', purpose = 'chat', userId }) {
  const originalName = path.basename(String(name || ''));
  const ext = String(originalName.split('.').pop() || '').toLowerCase();
  if (!originalName || !ext) throw Object.assign(new Error('文件名与扩展名必填'), { status: 400 });
  if (!ALLOWED_EXTS.includes(ext)) throw Object.assign(new Error(`不支持 .${ext}，可上传文档、表格、PDF及常用图片`), { status: 400 });
  ...
  try {
    ...
    const ret = q.run(`INSERT INTO uploaded_files(user_id,name,stored_name,ext,mime,size,purpose,file_path,file_url,extracted_text,extract_mode)
      VALUES(?,?,?,?,?,?,?,?,?,?,?)`, userId, originalName, storedName, ext, mime || '', buf.length, safePurpose, abs, fileUrl, extracted, mode);
    return { row: q.get(`SELECT * FROM uploaded_files WHERE tenant_id=? AND id=?`, curTenant(), ret.lastInsertRowid), buffer: buf };
  } catch (error) {
...
export function updateFileExtraction(id, text, mode) {
  q.run(`UPDATE uploaded_files SET extracted_text=?,extract_mode=? WHERE tenant_id=? AND id=?`, text || '', mode || '已处理', curTenant(), id);
  return q.get(`SELECT * FROM uploaded_files WHERE tenant_id=? AND id=?`, curTenant(), id);
}
...
export function listFiles(user, { purpose, limit = 50 } = {}) {
  ...
  if (purpose) { where += ' AND purpose=?'; params.push(String(purpose)); }
  params.push(Math.min(100, Math.max(1, Number(limit) || 50)));
  return q.all(`SELECT * FROM uploaded_files WHERE ${where} ORDER BY id DESC LIMIT ?`, ...params).map(publicFile);
}

export function ownedFile(id, user, allowManager = true) {
  const row = q.get(`SELECT * FROM uploaded_files WHERE tenant_id=? AND id=?`, curTenant(), id);
  if (!row) return null;
...
export function resolveAttachments(ids, user, max = 6) {
  const unique = [...new Set((Array.isArray(ids) ? ids : []).map(Number).filter(Boolean))].slice(0, max);
  return unique.map(id => ownedFile(id, user, true)).filter(Boolean).map(row => ({
    id: row.id,
    name: row.name,
    ext: row.ext,
    url: row.file_url,
    content: String(row.extracted_text || '').slice(0, 16000),
    readable: !!String(row.extracted_text || '').trim(),
  }));
}

export function resolveRequestedAttachments(ids, user, max = 6) {
  if (ids === undefined || ids === null) return [];
  if (!Array.isArray(ids)) throw Object.assign(new Error('文件列表格式不正确'), { status: 400 });
  const unique = [...new Set(ids.map(Number))];
  if (unique.length > max || unique.some(id => !Number.isInteger(id) || id <= 0)) {
    throw Object.assign(new Error(`一次最多引用${max}个有效文件`), { status: 400 });
  }
  const files = resolveAttachments(unique, user, max);
  if (files.length !== unique.length) throw Object.assign(new Error('部分文件不存在或无权访问'), { status: 404 });
  return files;
}

export function attachmentRefsForStorage(attachments = []) {
  return (Array.isArray(attachments) ? attachments : []).map(file => ({
    ...(Number.isInteger(Number(file?.id)) && Number(file.id) > 0 ? { id: Number(file.id) } : {}),
    ...
    ...(!file?.id && typeof file?.content === 'string' ? { content: file.content.slice(0, 16000) } : {}),
  }));
}

export function rehydrateMessageHistory(rows, user, { maxFiles = 12, maxChars = 30000 } = {}) {
