...
function fromPdf(buf) {
  ...
  while (out.join("").length < 14000) {
    ...
    try {
      text = zlib
        .inflateSync(slice, { maxOutputLength: MAX_PDF_STREAM_BYTES })
        .toString("latin1");
    } catch {
  ...
  return joined && readable > joined.length * 0.3
    ? joined.slice(0, 14000)
    : null;
}
...
export function extractText(buf, ext) {
  ...
  try {
    if (["txt", "md", "csv", "tsv", "json", "log"].includes(e))
      return buf.toString("utf8").slice(0, 16000);
