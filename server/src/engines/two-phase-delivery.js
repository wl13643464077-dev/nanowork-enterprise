...
// 5. generate / persist 失败都视为“未交付”，必须 release；
// 6. settle 失败不回滚已经交付的业务产物，也不虚构实扣金额，而是保留 hold 待对账。
...
export function twoPhaseBillingSummary({
  ...
  const chargedCredits = state === 'settled'
  ...
  return {
    ...
    heldCredits: state === 'held' || state === 'pending_reconciliation'
...
export function withImmediateTransaction(db, work) {
  if (!db?.exec || typeof work !== 'function') {
    throw new TypeError('withImmediateTransaction 需要数据库连接与同步工作函数');
  }
  db.exec('BEGIN IMMEDIATE');
  try {
    const result = work();
    if (result && typeof result.then === 'function') {
      throw new TypeError('业务落库事务必须是同步函数，不能在事务中等待外部异步操作');
    }
    db.exec('COMMIT');
    return result;
  } catch (error) {
    try { db.exec('ROLLBACK'); } catch { /* 事务可能已由底层失败关闭 */ }
    throw error;
  }
}

function attachBilling(error, billing, phase) {
  const target = error instanceof Error ? error : new Error(text(error) || '两阶段交付失败');
...
export async function executeHeldDelivery({
  ...
  if (!hold?.holdId) throw new TypeError('executeHeldDelivery 需要有效的预授权占扣');
  if (typeof generate !== 'function' || typeof persist !== 'function'
    || typeof settle !== 'function' || typeof release !== 'function') {
  ...
  try {
    ...
    let billing;
    try {
      const released = release(hold, typeof releaseNote === 'function'
  ...
  try {
    ...
    if (typeof onPendingReconciliation === 'function') {
      try {
        ...
        billing.reconciliationRecordError = text(markError?.message || markError);
      }
    }
  }
