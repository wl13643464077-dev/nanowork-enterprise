...
function splitSkills(library: EmployeeSkillLibrary) {
  ...
  const factory = uniqueSkills([
    ...
    ...(library.required || []),
    ...(library.optional || []).filter(
      skill => skill.origin !== 'legacy_learned' && skill.verificationStatus !== 'legacy_unverified',
    ),
  ...
  const historical = uniqueSkills([
    ...(library.historical || []),
    ...(library.pending || []),
    ...(library.pendingVerification || []),
    ...(library.historicalSkills || []),
    ...(library.optional || []).filter(
      skill => skill.origin === 'legacy_learned' || skill.verificationStatus === 'legacy_unverified',
    ),
    ...(library.learned || []).filter(
      skill => skill.origin === 'legacy_learned' || skill.verificationStatus === 'legacy_unverified',
    ),
...
function runtimeItems(runtime: EmployeeRuntime, domain: EmployeeWorkbenchDomain) {
  ...
  if (Number.isFinite(runtime.completedRuns))
    items.push({
      label: domain === 'content' ? '已采纳' : '已完成',
      value: `${runtime.completedRuns} 单`,
    });
  if (Number.isFinite(runtime.reviewPendingRuns))
    items.push({ label: '待审阅', value: `${runtime.reviewPendingRuns} 单` });
  ...
  if (Number.isFinite(runtime.avgDurationSeconds))
    items.push({ label: '平均用时', value: `${runtime.avgDurationSeconds}s` });
  ...
  if (last || lastTaskTime)
    items.push({ label: '最近运行', value: (last || lastTaskTime).replace('T', ' ').slice(0, 16) });
      ...
      {visible.map((capability, index) => {
        ...
        return (
          <article
            ...
            aria-label={`${capability.name}，${required ? '岗位必备能力' : '附加能力'}`}
          ...
          >
            ...
            <Switch
              className="ewb-locked-switch"
              size="small"
              checked={required || capability.enabled !== false}
              disabled
              aria-label={`${capability.name}已锁定启用`}
            />
          </article>
        );
      })}
...
function SkillCard({
  ...
  const historicalCatalogVerified = kind === 'historical' && skill.verificationLevel === 'catalog_contract_verified';
  const historicalLabel = historicalCatalogVerified ? '历史技能 · 目录契约已验证' : '历史技能 · 待核验';
  return (
      ...
      <div className="ewb-card-copy">
        ...
        <div className="ewb-card-meta">
          <Tag color={kind === 'factory' ? 'blue' : kind === 'historical' ? 'orange' : 'purple'}>
            {kind === 'factory' ? '出厂岗位 Skill' : kind === 'historical' ? historicalLabel : '企业自定义技能'}
          </Tag>
          {skill.source && <Tag>{skill.source}</Tag>}
          {kind === 'historical' && (
            <Tag icon={<CloseCircleOutlined />}>
              {historicalCatalogVerified
                ? '目录与执行注入已验证 · 业务效果待实测'
                : skill.defaultInjected === false
                  ? '未启用'
                  : '默认启用 · 使用前核验'}
            </Tag>
          )}
...
const KNOWN_CONFIG_FIELDS: WorkConfigField[] = [
  ...
  { key: 'timeoutSeconds', label: '执行超时（秒）', type: 'number' },
  {
    ...
    type: 'select',
    options: [
      { label: '老板审核', value: 'owner_review' },
      { label: '管理者审核', value: 'manager_review' },
...
function EmployeeWorkbenchInstance({ open, domain, idx, identityHint, onClose }: Props) {
  ...
  const applyProfile = useCallback(
    (normalized: EmployeeWorkbenchProfile) => {
  ...
  const historicalCatalogVerified =
    skills.historical.length > 0 &&
    skills.historical.every(skill => skill.verificationLevel === 'catalog_contract_verified');
  const configFields = profile ? effectiveConfigFields(profile.workConfig) : [];
  const capabilityCount =
    profile?.dispatch.lockedCapabilityCount ??
  ...
  useEffect(() => {
    ...
    const pull = () =>
      api
        .get(`/marshals/tasks/${taskId}/status`)
  ...
  const dispatchTab =
    ...
    (() => {
      ...
      const titlePlaceholder =
        text(guidance.titlePlaceholder) || `例如：请${profile.identity.name}按岗位职责完成一项具体交付`;
      const requirementLabel = text(guidance.requirementLabel) || `请提供${profile.identity.name}开工所需的真实材料`;
      const requirementPlaceholder =
        text(guidance.requirementPlaceholder) ||
        `请逐项提供必要输入；缺失项明确写“暂无”，并说明时间范围、限制条件和交付用途。`;
      return (
        <div className="ewb-section">
          <SectionHeading
            title="单独派活"
            description="先按这个岗位的操作指引给齐材料；开工后会锁定全部必备能力、提示词和工作配置。"
          />
          <Alert
            ...
            description={
              profile.dispatch.snapshotNotice ||
              profile.dispatch.boundary ||
              '任务记录会保存能力、提示词与配置快照，便于追溯。'
            }
          />
          ...
          <section className="ewb-dispatch-guide" aria-label={`${profile.identity.name}派活操作指引`}>
            <div className="ewb-dispatch-guide-intro">
              ...
              <p>
                {text(guidance.intro) ||
                  `${profile.identity.name}会严格按岗位档案处理专项问题；材料不足时会标出缺口，不会猜测成事实。`}
              </p>
            </div>
          ...
          <div className="ewb-panel">
            <Form form={dispatchForm} layout="vertical" requiredMark={false} onFinish={submitDispatch}>
              <Form.Item
                ...
                label={titleLabel}
                rules={[
                  { required: true, message: `请写明要交给${profile.identity.name}的具体问题` },
                  { min: 4, message: '任务目标至少4个字' },
                ]}
              >
              ...
              <Form.Item
                ...
                rules={[
                  { required: true, message: `请补充${profile.identity.name}需要的真实材料` },
                  { min: 12, message: '请至少写12个字，避免员工只能猜测' },
                ]}
              >
                <Input.TextArea
                  rows={7}
              ...
              <Form.Item
                label={text(guidance.imageLabel) || `${profile.identity.name}相关图片证据（可选）`}
                extra={
                  text(guidance.imageHelp) ||
                  '如图片有助于岗位判断，可附一张PNG、JPEG或WebP；图片只随本次模型调用使用，任务快照仅保存文件名、大小与哈希。'
                }
              >
                <div className="ewb-image-input">
                  <label htmlFor="employee-workbench-dispatch-image">
                    <PictureOutlined />
                    <span>{dispatchImage ? '更换图片' : '选择 PNG / JPEG / WebP'}</span>
                    <input
                      id="employee-workbench-dispatch-image"
                      ref={dispatchImageInputRef}
                      type="file"
              ...
              <Form.Item name="dueAt" label="截止时间（可选）">
                <Input type="datetime-local" disabled={!dispatchAvailable} />
              </Form.Item>
              <Button
                type="primary"
                htmlType="submit"
          ...
          {domain === 'restaurant' && restaurantTask && (
            <Alert
              showIcon
              type={restaurantTask.failed ? 'error' : restaurantTask.status === '已完成' ? 'success' : 'info'}
              ...
              description={
                ...
                >
                  <Steps
                    ...
                    items={(restaurantTask.flow || ['已派发', 'AI生成中', '待审阅', '已完成']).map((step: string) => ({
                      title: step,
                    }))}
                  />
          ...
          {domain === 'content' && (
            <section className="ewb-run-center" aria-label="内容员工近期任务与结果">
              <SectionHeading
                title="近期任务与结果"
                description="派活后可在这里持续查看生成状态、完整结果、失败说明和人工审阅记录。"
                extra={
                  <Button size="small" icon={<ReloadOutlined />} loading={runsLoading} onClick={() => void loadRuns()}>
                    刷新
                  </Button>
                }
              />
              ...
              {selectedRun && (
                <article className="ewb-run-detail">
                  <div className="ewb-run-detail-head">
                    <div>
                      <Space size={6} wrap>
                        <Tag color={RUN_STATUS_COLOR[runStatus(selectedRun)] || 'default'}>
                  ...
                  {selectedRun.billing && (
                    <Alert
                      ...
                      showIcon
                      message={
                        selectedRun.billing.state === 'held'
                  ...
                  {selectedRun.contract?.valid === true && (
                    <Alert
                      type="success"
                      showIcon
                      message="岗位原生输出契约校验通过"
                      description={`已覆盖岗位必需字段；${
                  ...
                  {selectedRun.review && (
                    <div className="ewb-run-review-record">
                      ...
                      {selectedRun.review.contentId && (
                        <span>
                          已形成可发布内容 #{selectedRun.review.contentId}，可在内容列表继续登记；系统未自动发布
                        </span>
                      )}
  ...
  const skillsTab = profile && (
    <div className="ewb-section">
      <SectionHeading
        ...
        description={
          historicalCatalogVerified
            ? '严格区分出厂岗位 Skill、已完成目录与执行注入验证的历史技能，以及本企业自定义技能；历史技能的真实业务效果仍需实测。'
            : '严格区分出厂岗位 Skill、历史待核验技能和本企业自定义技能。'
        }
        extra={
      ...
      <div className="ewb-skill-group">
        <h4>{historicalCatalogVerified ? '历史技能 · 目录契约已验证' : '历史技能 · 待核验'}</h4>
        {skills.historical.length ? (
          <div className="ewb-skill-list">
            {skills.historical.map((skill, index) => (
              <SkillCard
                key={skill.id || skill.key || `${skillTitle(skill)}-${index}`}
                skill={skill}
                kind="historical"
          ...
          <HonestEmpty description="没有历史技能记录" />
        )}
      </div>
      <div className="ewb-skill-group">
        <h4>本企业自定义技能</h4>
            ...
            <section className="ewb-overview">
              <div className="ewb-overview-copy">
                ...
                <div className="ewb-overview-tags">
                  {(profile.identity.group || departmentName(profile.identity)) && (
                    <Tag>{profile.identity.group || departmentName(profile.identity)}</Tag>
                  )}
                  ...
                  <Tag color="blue" icon={<LockOutlined />}>
                    {capabilityCount} 项必备能力锁定
                  </Tag>
                </div>
              </div>
              {!!liveRuntimeItems.length && (
                <div className="ewb-runtime" aria-label="员工真实运行统计">
                  {liveRuntimeItems.map(item => (
                    <div className="ewb-runtime-item" key={item.label}>
