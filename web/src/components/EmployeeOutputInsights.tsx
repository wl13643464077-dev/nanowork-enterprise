...
export default function EmployeeOutputInsights({ start, end }: Props) {
  ...
  const [detail, setDetail] = useState<{ open: boolean; loading: boolean; data: any; context: any }>({
    open: false,
    loading: false,
    data: null,
    context: null,
  ...
  useEffect(() => {
    ...
    api
      .get(`/analysis/employee-outputs?${query}`)
      .then(value => {
  ...
  const openDetail = (row: any) => {
    const labelFor = (items: any[], value: string, fallback: string) =>
    ...
    const context = {
      ...
      employee: labelFor(
        ...
        filters.employee,
        filters.employee ? filters.employee : '全部员工',
      ),
      source: labelFor(
    ...
    api
      .get(`/analysis/employee-outputs/drill/${row.source}/${row.id}`)
      .then(value => {
        if (detailRequestRef.current === detailRequestId)
          setDetail({ open: true, loading: false, data: value, context });
      })
      .catch(() => {
        if (detailRequestRef.current === detailRequestId)
  ...
  return (
    <Panel
      title={
        <Space size={8}>
          <AuditOutlined style={{ color: 'var(--ui-accent)' }} />
          数字员工产出透视
        </Space>
      }
      extra={
        <Space size={6} wrap>
          <Tag color={data?.dataset?.isDemoEnvironment ? 'orange' : data ? 'blue' : 'default'}>
            {data?.dataset?.kind || '数据待加载'}
          </Tag>
          <Tag>
            {start} 至 {end}
          </Tag>
    ...
    >
      ...
      <Space wrap size={[8, 8]} style={{ marginBottom: 12 }}>
        <Tooltip title="门店数据由当前登录企业的租户边界固定，不能跨店查看">
          <Select
            value={data?.dataset?.store?.name || '当前门店'}
            disabled
            style={{ width: 170 }}
            options={[
              {
                value: data?.dataset?.store?.name || '当前门店',
                label: `门店 · ${data?.dataset?.store?.name || '当前门店'}`,
              },
            ]}
          />
        </Tooltip>
        ...
        <Select
          ...
          options={[
            ...
            { value: 'domain', label: '按能力域' },
            { value: 'group', label: '按分部' },
        ...
        {(filters.domain !== 'all' ||
          ...
          <Button
            ...
            onClick={() => {
              setPage(1);
              setFilters(prev => ({ ...initialFilters, dimension: prev.dimension }));
            }}
          >
            清空筛选
          </Button>
      ...
      <Spin spinning={loading}>
        {error && !loading ? (
          ...
          <Row gutter={[12, 12]}>
            <Col xs={24} xl={9}>
              <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--ui-text-2)', marginBottom: 6 }}>
                透视汇总 · 点击行可继续筛选
              </div>
              <Table
                size="small"
                ...
                pagination={false}
                locale={{
                  emptyText: <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="当前范围暂无运行记录" />,
                }}
            ...
            <Col xs={24} xl={15}>
              ...
              <Table
                ...
                scroll={{ x: 860 }}
                locale={{
                  emptyText: <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="当前筛选暂无运行记录" />,
                }}
                ...
                columns={[
                  {
                    ...
                    dataIndex: 'createdAt',
                    width: 126,
                    render: (value: string) => String(value || '').slice(0, 16),
                  },
                  {
                    title: '来源',
                  ...
                  { title: '能力域', dataIndex: 'abilityDomainLabel', width: 124, ellipsis: true },
                  {
                    ...
                    render: (value: string, row: any) => (
                      <Tooltip title={row.group}>
                        {row.employeeIdx != null ? `${row.employeeIdx} · ` : ''}
                        {value}
                      </Tooltip>
                    ),
                  },
                  ...
                  {
                    title: '证据',
                    dataIndex: 'evidenceKind',
                    width: 105,
                    render: (value: string, row: any) => (
                      <Tooltip title={row.evidenceLabel}>
                        <Tag color={evidenceColor(value)}>{value}</Tag>
                      </Tooltip>
                    ),
                  },
                ]}
      ...
      <Modal
        ...
        title={
          <Space>
            <span>运行记录穿透</span>
            <Tag color="blue">{detail.data?.evidenceKind || '加载中'}</Tag>
          </Space>
        }
      >
        {detail.loading ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <Panel title="本次穿刺的筛选快照与计算口径">
              ...
              <div style={{ marginTop: 8, color: 'var(--ui-text-2)', fontSize: 12, lineHeight: 1.7 }}>
                统计单位：{detail.context?.calculation?.unit || '一条持久化运行记录'}； 产出率：
                {detail.context?.calculation?.outputRate || '已有产出的记录数 ÷ 当前筛选后的运行记录数'}。
            ...
            {(detail.data.record?.requirement || detail.data.record?.inputSummary || detail.data.record?.inputs) && (
              <Panel title="输入与要求">
                {detail.data.record?.requirement || detail.data.record?.inputSummary ? (
                  <div style={{ whiteSpace: 'pre-wrap', color: 'var(--ui-text-2)', lineHeight: 1.7 }}>
                    {detail.data.record.requirement || detail.data.record.inputSummary}
                  </div>
                ) : null}
                {detail.data.record?.inputs && Object.keys(detail.data.record.inputs).length > 0 && (
                  ...
                  >
                    {jsonPreview(detail.data.record.inputs)}
                  </pre>
                )}
              </Panel>
            )}
            <Panel title="运行产出">
              ...
              {detail.data.output?.artifactUrl && safeUrl(detail.data.output.artifactUrl) !== '#' && (
                <Button
                  ...
                  href={safeUrl(detail.data.output.artifactUrl)}
                  target="_blank"
                  rel="noreferrer"
                >
                  查看媒体产出证据
            ...
            {detail.data.output &&
              (detail.data.output.assumptions || detail.data.output.evidence || detail.data.output.provenance) && (
            ...
            {detail.data.execution?.snapshot && (
              <Panel title="完整执行快照">
                <Descriptions size="small" bordered column={{ xs: 1, sm: 2 }} style={{ marginBottom: 10 }}>
                  <Descriptions.Item label="岗位档案快照">
                    {detail.data.execution.profileVersion ? '已保存' : '未记录'}
                  </Descriptions.Item>
