...
const formatReviewTime = (value?: string | null) => String(value || '').replace('T', ' ').slice(0, 16);
...
export function MediaReviewTags({ job }: { job: MediaJobView }) {
  ...
  const business = job.businessStatus || job.reviewStatus || (
    technical === '成功' ? '待人工验收' : '未进入人工验收'
  );
  ...
  return (
    <div style={{ display: 'flex', gap: 4, alignItems: 'center', flexWrap: 'wrap' }}>
      ...
      <Tag color={job.businessUsable ? 'success' : job.reviewRequired || technical === '成功' ? 'gold' : 'default'} style={{ margin: 0 }}>
        业务：{business}
      </Tag>
      {job.mediaType && <Tag color="purple" style={{ margin: 0 }}>{job.mediaType}{job.mimeType ? ` · ${job.mimeType}` : ''}</Tag>}
      {billing && (
        <Tooltip title={billing.detail}>
          <Tag color={billing.color} style={{ margin: 0 }}>{billing.label}</Tag>
        </Tooltip>
      )}
    </div>
  );
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
        <Checkbox disabled={job.canDelete === false} checked={selected} onChange={event => onToggle(event.target.checked)}>
          选择
        </Checkbox>
      ...
      {kind === 'image' && url ? (
        <img src={url} alt={job.prompt || '生成图片预览'} style={{ width: '100%', height: 110, objectFit: 'cover', borderRadius: 6 }} />
      ) : kind === 'video' && url ? (
        <video src={url} controls preload="metadata" style={{ width: '100%', height: 110, objectFit: 'cover', borderRadius: 6, background: '#000' }} />
      ) : (
        <div style={{
          height: 110,
      ...
      <div style={{ fontSize: 11.5, color: 'var(--ui-text-2)', marginTop: 7, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
        {job.prompt || '未填写提示词'}
      </div>
      ...
      {url && (
        <a href={url} target="_blank" rel="noreferrer" style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 10.5, marginTop: 5 }}>
          <LinkOutlined /> 打开源文件
        </a>
      )}
