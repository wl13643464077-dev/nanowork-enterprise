...
export default function Analysis() {
  ...
  const [subDrill, setSubDrill] = useState<{ open: boolean; title: string; rows: any[]; loading: boolean }>({
    open: false,
    title: '',
    rows: [],
    loading: false,
  ...
  const openDayOrders = (date: string) => {
    ...
    api
      .get(`/analysis/drill/orders?date=${date}`)
      .then(d => setSubDrill({ open: true, title: d.title, rows: d.rows || [], loading: false }));
  };
  const openBucketOrders = (b: any) => {
    ...
    api
      .get(`/analysis/drill/orders?${qs2()}&lo=${lo}${hi !== '' ? `&hi=${hi}` : ''}`)
      .then(d =>
        setSubDrill({ open: true, title: `客单价 ${b.label} 订单（${b.n}笔）`, rows: d.rows || [], loading: false }),
      );
  };
  ...
  const openSourceSample = (card: any) => {
    ...
    api
      .get(`/analysis/source-samples/${card.key}`)
  ...
  const openVisualDetail = (kind: string, params: Record<string, any> = {}, fallbackTitle = '明细追溯') => {
    ...
    api
      .get(`/analysis/visual-drill/${kind}?${qs}`)
  ...
  useEffect(() => {
    ...
    const apply = (setter: (value: any) => void) => (value: any) => {
      if (requestId === analysisRequestRef.current) setter(value);
    };
    const qs = `?start=${range[0].format('YYYY-MM-DD')}&end=${range[1].format('YYYY-MM-DD')}`;
  ...
  const masked = (
    <Tooltip title="仅老板/总监可见">
      <span style={{ color: '#b6bdc9', cursor: 'help' }}>–</span>
    </Tooltip>
  );

  // ===== 增长机会 → 转任务 =====
  const toTask = async (it: any) => {
    const ret = await api.post('/execution/tasks', {
      title: (it.suggestion || it.issue || '').slice(0, 30),
      type: '其他',
      priority: '高',
    });
  ...
  const exportWeekly = () => {
    ...
    const lines = [
      ...
      '',
      '【本周核心指标】',
      ...Object.entries(weekly.metrics || {}).map(
        ([k, m]: any) =>
      ...
      '',
      '【本周爆款内容】',
      ...((weekly.topContents || []).length
        ? (weekly.topContents || []).map((c: any) => `· ${c.title}（带来线索 ${c.effect_leads}）`)
        : ['· 暂无']),
      '',
      '【合伙人周榜】',
      ...((weekly.partnerBoard || []).length
        ? (weekly.partnerBoard || []).map((p: any, i: number) => `${i + 1}. ${p.name}（积分 ${p.total}）`)
        : ['· 暂无']),
      '',
      '【流失原因】',
      ...((weekly.lostReasons || []).length
        ? (weekly.lostReasons || []).map((l: any) => `· ${l.lost_reason}：${l.n} 人`)
        : ['· 暂无']),
      '',
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <Row gutter={[12, 12]}>
        <Col xs={12} md={8} xl={4}>
          <StatCard
            ...
            color="var(--ui-accent)"
            label="总营收"
            value={overview.revenue !== undefined ? fmtMoney(overview.revenue) : '-'}
            trend={overview.revenueWow}
            trendLabel="较上期"
          ...
          <StatCard
            ...
            value={
              overview.grossMargin === null
                ? masked
                : overview.grossMargin !== undefined
          ...
          <StatCard
            ...
            value={
              overview.marketingCost === null
                ? masked
                : overview.marketingCost !== undefined
                  ? fmtMoney(overview.marketingCost)
                  : '-'
            }
            onClick={() => openDrill('marketing')}
          />
        </Col>
      </Row>
              ...
              option={
                {
                  ...
                  legend: {
                    ...
                    itemWidth: 14,
                    itemHeight: 8,
                    textStyle: { fontSize: 11, color: 'var(--ui-text-2)' },
                  },
                  grid: baseGrid,
                  xAxis: {
                    type: 'category',
                    data: trend.rows.map((x: any) => x.date.slice(5)),
                    ...axisStyle,
                    boundaryGap: false,
                  },
                  ...
                  series: [
                    {
                      ...
                      areaStyle: {
                        color: {
                          ...
                          y2: 1,
                          colorStops: [
                            { offset: 0, color: 'rgba(43,109,239,.22)' },
          ...
          <Panel title="渠道业绩占比">
            <div style={{ display: 'flex', alignItems: 'center' }}>
              <div style={{ flex: 1.1, minWidth: 0 }}>
                <Chart
                  height={216}
                  onClick={(p: any) => p?.name && openVisualDetail('channel', { channel: p.name }, `${p.name}渠道明细`)}
                  option={
                    {
                      tooltip: {
                        trigger: 'item',
                      ...
                      color: CHART_COLORS,
                      title: {
                        text: fmtWan(channelTotal),
                        subtext: '渠道总业绩',
                        left: 'center',
                        top: '37%',
                        textStyle: { fontSize: 16, fontWeight: 700, color: 'var(--ui-text)' },
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                {channels.map((c, i) => (
                  <button
                    type="button"
                    key={c.channel}
                    onClick={() => openVisualDetail('channel', { channel: c.channel }, `${c.channel}渠道明细`)}
                    style={{
                      ...buttonResetStyle,
                      display: 'flex',
                      alignItems: 'center',
                  ...
                  >
                    ...
                    <span style={{ color: 'var(--ui-muted)', width: 40, textAlign: 'right' }}>
                      {(((c.amount || 0) / (channelTotal || 1)) * 100).toFixed(1)}%
                    </span>
                  </button>
                ))}
        ...
        <Col xs={24} lg={7}>
          <Panel title="客群结构分析">
            <div style={{ display: 'flex', alignItems: 'center' }}>
              <div style={{ flex: 1.1, minWidth: 0 }}>
                <Chart
                  height={148}
                  onClick={(p: any) =>
                    openVisualDetail('identity', { identity: p?.name }, `${p?.name || '未知'}客群明细`)
                  }
                  option={
                    {
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                {(customers.byIdentity || []).map((x: any, i: number) => (
                  <button
                    ...
                    key={x.k || i}
                    onClick={() =>
                      openVisualDetail('identity', { identity: x.k || '未知' }, `${x.k || '未知'}客群明细`)
                    }
                    style={{
                      ...buttonResetStyle,
                      display: 'flex',
            ...
            <div style={{ marginTop: 8, background: 'var(--ui-surface-2)', borderRadius: 8, padding: '8px 10px' }}>
              <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--ui-text)', marginBottom: 4 }}>
                成交路径分布
              </div>
              {(customers.byPath || []).map((x: any) => (
                <button
                  type="button"
                  key={x.k}
                  onClick={() => openVisualDetail('path', { path: x.k }, `${x.k}路径明细`)}
                  style={{
                    ...buttonResetStyle,
                    display: 'flex',
                    alignItems: 'center',
                ...
                >
                  ...
                  />
                  <span
                    style={{ fontSize: 11.5, color: 'var(--ui-text)', fontWeight: 600, width: 36, textAlign: 'right' }}
                  >
                    {x.n}人
                  </span>
                </button>
      ...
      <Row gutter={[12, 12]}>
        <Col xs={24} lg={9}>
          <Panel title="商品销售TOP10">
            <Table
              ...
              dataSource={products}
              onRow={(r: any) => ({
                onClick: () => openVisualDetail('product', { product: r.product }, `${r.product}销售明细`),
        ...
        <Col xs={24} lg={8}>
          <Panel title="区域销售分布">
            <Chart
              height={386}
              onClick={(p: any) => p?.name && openVisualDetail('region', { region: p.name }, `${p.name}区域明细`)}
              option={
                {
                  tooltip: {
                    formatter: (p: any) =>
                      `${p.name}<br/>销售额 ${fmtMoney(p.value)} · 占比 ${p.data?.share}%<br/>订单 ${p.data?.orders} 单`,
                  },
          ...
          <Panel title="经营健康度">
            <div style={{ textAlign: 'center', padding: '4px 0 12px' }}>
              ...
              <div style={{ marginTop: 10 }}>
                {health.level && (
                  <Tag color={levelColor[health.level] || 'blue'} style={{ fontSize: 12, padding: '1px 12px' }}>
                    {health.level}
                  </Tag>
                )}
              </div>
            ...
            {(health.subs || []).map((s: any) => (
              <button
                type="button"
                key={s.key}
                onClick={() => openVisualDetail('health', { part: s.key }, `${s.name}评分依据`)}
                style={{
                  ...buttonResetStyle,
                  padding: '6px 0',
              ...
              >
                ...
                <Progress
                  percent={Math.min(s.score, 100)}
                  showInfo={false}
                  size="small"
                  strokeColor={scoreColor(s.score)}
                />
              </button>
            ))}
          </Panel>
      ...
      <Row gutter={[12, 12]}>
            ...
            <Chart
              ...
              onClick={(p: any) => {
                ...
                if (row?.date)
                  openVisualDetail(
                    'kpi-day',
                    { date: row.date, metric: p.seriesName },
                    `${row.date} ${p.seriesName}明细`,
                  );
              }}
              option={
                {
        ...
        <Col xs={24} lg={8}>
          <Panel
            title={
              <>
                <BulbOutlined style={{ color: 'var(--ui-accent)' }} /> 增长机会洞察
              </>
            }
            extra={
          ...
          >
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10, maxHeight: 392, overflow: 'auto' }}>
              {insights.length === 0 && (
                <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="暂无洞察，经营状态良好" />
              )}
              {insights.map((it, i) => (
                <div
                  role="button"
                  tabIndex={0}
                  key={i}
                  onClick={() => openVisualDetail('insight', { dimension: it.dimension }, `${it.dimension}依据`)}
                  onKeyDown={e => {
                    if (e.target !== e.currentTarget || (e.key !== 'Enter' && e.key !== ' ')) return;
                    e.preventDefault();
                    openVisualDetail('insight', { dimension: it.dimension }, `${it.dimension}依据`);
                  }}
                  style={{
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'flex-start', gap: 6 }}>
                    <Tag color={DIM_TAG[i % DIM_TAG.length]} style={{ marginRight: 0, flexShrink: 0 }}>
                      {it.dimension}
                    </Tag>
                    <span style={{ fontSize: 12.5, fontWeight: 600, color: 'var(--ui-text)', lineHeight: 1.6 }}>
                      {it.issue}
                    </span>
                  </div>
        ...
        <Col xs={24} lg={8}>
          <Panel
            title={
              <>
                <FileTextOutlined style={{ color: 'var(--ui-accent)' }} /> 经营周报
              </>
            }
            extra={
              weekly?.week && (
          ...
          >
            {!weekly ? (
              <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="暂无周报">
                <Button type="primary" size="small" loading={gen} icon={<ThunderboltOutlined />} onClick={genWeekly}>
                  生成本周周报
                </Button>
              </Empty>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                    ...
                    <Col span={8} key={k}>
                      <div style={{ background: 'var(--ui-surface-2)', borderRadius: 8, padding: '8px 10px' }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'baseline', gap: 4, flexWrap: 'wrap' }}>
                          <span style={{ fontSize: 14.5, fontWeight: 700, color: 'var(--ui-text)' }}>
                            {k.includes('金额') ? fmtWan(m?.value || 0) : (m?.value ?? 0)}
                          </span>
                          <span
                            style={{
                              fontSize: 10.5,
                              color: (m?.wow ?? 0) >= 0 ? '#16a34a' : '#ef4444',
                              whiteSpace: 'nowrap',
                            }}
                          >
                            {(m?.wow ?? 0) >= 0 ? <ArrowUpOutlined /> : <ArrowDownOutlined />}
                            {Math.abs(m?.wow ?? 0)}%
                          </span>
                        </div>
                      </div>
                ...
                <div style={{ background: 'var(--ui-surface)', borderRadius: 8, padding: '8px 10px' }}>
                  ...
                  {(weekly.nextWeek?.actions || []).map((a: string, i: number) => (
                    <div key={i} style={{ fontSize: 12, color: 'var(--ui-text-2)', marginTop: 4, lineHeight: 1.6 }}>
                      {i + 1}. {a}
                    </div>
                  ))}
                </div>
                <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', alignItems: 'center' }}>
                  {weekly.generatedAt && (
                    <span style={{ fontSize: 11, color: 'var(--ui-muted)', marginRight: 'auto' }}>
                      生成于 {String(weekly.generatedAt).slice(0, 16)}
                    </span>
                  )}
                  <Button size="small" icon={<ReloadOutlined />} loading={gen} onClick={genWeekly}>
                    重新生成周报
                  </Button>
                  <Button size="small" type="primary" icon={<DownloadOutlined />} onClick={exportWeekly}>
                    导出
                  </Button>
                </div>
              </div>
      ...
      <Panel
        ...
        extra={
          <span style={{ fontSize: 12, color: 'var(--ui-muted)' }}>每个结论都能回查数据出处，点击卡片查看真实样本</span>
        }
      >
        {sourceMapError ? (
                ...
                >
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', gap: 8 }}>
                    ...
                    <span style={{ fontSize: 15, fontWeight: 700, color: 'var(--ui-accent)', whiteSpace: 'nowrap' }}>
                      {c.count ?? 0} <span style={{ fontSize: 11, fontWeight: 400, color: 'var(--ui-muted)' }}>条</span>
                    </span>
                  </div>
                  <div
                    style={{
      ...
      >
        {sourceDetail.loading ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            <Table
              size="small"
              rowKey={(r: any, i?: number) => r.id || r.date || i}
              dataSource={sourceDetail.data?.rows || []}
              pagination={{ pageSize: 8, size: 'small' }}
              locale={{
                emptyText: <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="当前权限范围暂无样本数据" />,
              }}
              onRow={(r: any) => ({
                onClick: () => r.lead_id && setCustId(r.lead_id),
            ...
            {(sourceDetail.data?.logs || []).length > 0 && (
              <Panel title="最近补录/操作记录">
                <Table
                  size="small"
                  rowKey={(r: any, i?: number) => `${r.created_at}-${i}`}
                  ...
                  columns={[
                    ...
                    { title: '内容', dataIndex: 'target', ellipsis: true, render: (v: any) => renderSourceCell(v) },
                    {
                      ...
                      width: 140,
                      render: (v: any) => String(v || '').slice(0, 16),
                    },
                  ]}
                />
              </Panel>
              ...
              <Table
                ...
                columns={[
                  { title: '日期', dataIndex: 'date', width: 100 },
                  {
                    ...
                    align: 'right',
                    render: (v: any) => <b style={{ color: 'var(--ui-accent)' }}>{fmtMoney(v || 0)}</b>,
                  },
                ...
                columns={[
                  {
                    ...
                    render: (v: any, r: any) => (
                      <span>
                        <b>{v || '—'}</b>{' '}
                        <span style={{ fontSize: 11, color: 'var(--ui-muted)' }}>{r.identity_tag || ''}</span>
                      </span>
                    ),
                  },
                  ...
                  },
                  {
                    ...
                    width: 66,
                    render: (v: any) => (
                      <Tag color={v === '复购' ? 'purple' : 'blue'} style={{ margin: 0 }}>
                        {v}
                      </Tag>
                    ),
                  },
              ...
              <Table
                ...
                columns={[
                  ...
                  { title: '当日营收', dataIndex: 'deal_amount', align: 'right', render: (v: any) => fmtMoney(v || 0) },
                  {
                    ...
                    align: 'right',
                    render: (r: any) =>
                      r.deal_amount > 0 ? `${Math.round((r.marketing_cost / r.deal_amount) * 1000) / 10}%` : '—',
                  },
                  { title: '新增线索', dataIndex: 'new_leads', align: 'right', width: 80 },
                  {
                    title: '获客成本',
                    width: 90,
                    align: 'right',
                    render: (r: any) => (r.new_leads > 0 ? fmtMoney(Math.round(r.marketing_cost / r.new_leads)) : '—'),
                  },
                ]}
              />
            )}
      ...
      <Modal
        ...
        width={720}
        footer={null}
        onCancel={() => setSubDrill({ open: false, title: '', rows: [], loading: false })}
        title={
          <span>
            {subDrill.title}{' '}
            <Tag color="blue" style={{ fontSize: 10.5 }}>
      ...
      >
        ...
        ) : subDrill.rows.length === 0 ? (
          <Empty description="该范围暂无订单" />
        ) : (
          <>
            <Table
              size="small"
              rowKey="id"
              dataSource={subDrill.rows}
              pagination={{ pageSize: 8, size: 'small' }}
              onRow={(r: any) => ({ onClick: () => r.lead_id && setCustId(r.lead_id), style: { cursor: 'pointer' } })}
              columns={[
                {
            ...
            <div style={{ marginTop: 8, fontSize: 11.5, color: 'var(--ui-muted)' }}>
              点击任意一行打开客户完整档案（生命周期 + 全部订单 + 跟进时间线）。
            </div>
          </>
        )}
      </Modal>
      ...
      >
        ...
        <Form form={uploadForm} layout="vertical" initialValues={{ date: dayjs() }}>
          ...
          <Row gutter={10}>
            <Col span={12}>
              <Form.Item name="new_leads" label="新增线索">
                <InputNumber min={0} precision={0} style={{ width: '100%' }} />
              </Form.Item>
            </Col>
            <Col span={12}>
              <Form.Item name="invited" label="已邀约">
                <InputNumber min={0} precision={0} style={{ width: '100%' }} />
              </Form.Item>
            </Col>
            <Col span={12}>
              <Form.Item name="arrived" label="已到店">
                <InputNumber min={0} precision={0} style={{ width: '100%' }} />
              </Form.Item>
            </Col>
            <Col span={12}>
              <Form.Item name="content_count" label="内容产出数">
                <InputNumber min={0} precision={0} style={{ width: '100%' }} />
              </Form.Item>
            </Col>
            <Col span={12}>
              <Form.Item name="active_partners" label="活跃合伙人">
                <InputNumber min={0} precision={0} style={{ width: '100%' }} />
              </Form.Item>
            </Col>
            <Col span={12}>
              <Form.Item name="marketing_cost" label="营销费用（管理层）">
