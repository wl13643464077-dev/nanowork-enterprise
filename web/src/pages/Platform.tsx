...
export default function Platform() {
  ...
  useEffect(() => {
    const frame = requestAnimationFrame(loadAll);
  ...
  const openApprove = (t: any) => {
    ...
    aForm.setFieldsValue({
      modules: t.modules ? parseModules(t.modules) : ALL_MODULES,
      plan: t.plan || '标准版',
      seatLimit: t.seat_limit || 50,
      grantCredits: t.status === '待审核' ? 10000 : 0,
    });
  };
  const submitApprove = () =>
    aForm.validateFields().then(v => {
      api.post(`/platform/tenants/${approve.id}/approve`, v).then(() => {
        message.success(approve.status === '待审核' ? '账号已开通' : '已更新');
        setApprove(null);
        loadAll();
      });
    });
  const setStatus = (t: any, status: string) =>
    api.post(`/platform/tenants/${t.id}/update`, { status }).then(() => {
  ...
  const directCredit = (t: any) => {
    ...
    Modal.confirm({
      ...
      onOk: () => {
        if (val)
          api.post(`/platform/tenants/${t.id}/credits`, { delta: val, note: '平台直充' }).then(() => {
            message.success('已处理');
            loadAll();
          });
      },
  ...
  const confirmOrder = (o: any) =>
    Modal.confirm({
      title: '确认收到款项？',
      content: `确认后将向「${o.tenant_name}」充入 ${o.credits.toLocaleString()} 积分（¥${o.price_yuan}）`,
      okText: '确认到账',
      onOk: () =>
        api.post(`/platform/recharge-orders/${o.id}/confirm`, {}).then(() => {
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--ui-surface-2)' }}>
      <div
        style={{
          height: 56,
          background: 'linear-gradient(90deg,var(--ui-border-strong),var(--ui-primary-strong))',
          display: 'flex',
          alignItems: 'center',
      ...
      <div style={{ padding: 20, display: 'flex', flexDirection: 'column', gap: 16, maxWidth: 1400, margin: '0 auto' }}>
        ...
        <Segmented
          ...
          options={['开通管理', '概览', '运营分析', '积分流水', '客户企业', '充值订单', '套餐管理']}
        />
        ...
        {tab === '开通管理' && (
          <Panel
            ...
            extra={
              <Tag color={pendingTenants.length ? 'gold' : 'green'}>
                {pendingTenants.length ? `${pendingTenants.length} 家待开通` : '暂无待开通'}
              </Tag>
            }
          >
            <Table
                ...
                },
                {
                  ...
                  render: (v: string, r: any) => (
                    <span>
                      {v || '-'}
                      <br />
                      <span style={{ fontSize: 11, color: 'var(--ui-muted)' }}>{r.phone || '-'}</span>
                    </span>
                  ),
                },
                {
          ...
          <>
            <Row gutter={[12, 12]}>
              ...
              <Col xs={12} md={6}>
                <StatCard
                  ...
                  color="var(--ui-accent)"
                  label="付费企业 / ARPU"
                  value={ana.summary.paying}
                  suffix={`家 · ¥${(ana.summary.arpu || 0).toLocaleString()}`}
                />
              </Col>
              <Col xs={12} md={6}>
                <StatCard
            ...
            <Row gutter={[12, 12]}>
              <Col xs={24} lg={10}>
                ...
                >
                  {(ana.revByMonth || []).length === 0 ? (
                    <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="暂无充值" />
                  ) : (
              ...
              <Col xs={24} lg={14}>
                <Panel
                  title={
                    <>
                      <ShopOutlined style={{ color: 'var(--ui-accent)' }} /> 企业健康度 / 续费预警
                    </>
                  }
                  extra={
                    ana.summary.lowBalance > 0 && <Tag color="red">{ana.summary.lowBalance} 家积分见底，建议催充</Tag>
                  }
                >
                  <Table
                    size="small"
                    rowKey="id"
                    dataSource={ana.tenants}
                    ...
                    columns={[
                      {
                        ...
                        render: (v: string, r: any) => (
                          <span>
                            <b>{v}</b> <Tag style={{ fontSize: 10 }}>{r.plan}</Tag>
                          </span>
                        ),
                      },
                      {
                        ...
                        render: (v: string) => (
                          <Tag
                            color={
                              v === '活跃' ? 'green' : v === '一般' ? 'blue' : v === '流失预警' ? 'red' : 'default'
                            }
                          >
                            {v}
                          </Tag>
        ...
        {(tab === '概览' || tab === '客户企业') && (
          <Panel
            title={
              <>
                <ShopOutlined style={{ color: 'var(--ui-accent)' }} /> 客户企业（租户）
              </>
            }
          >
            <Table
              ...
              columns={[
                ...
                },
                {
                  ...
                  dataIndex: 'credits',
                  align: 'right',
                  render: (v: number) => <b style={{ color: 'var(--ui-accent)' }}>{(v ?? 0).toLocaleString()}</b>,
                },
                {
                  title: '累计充值',
                  dataIndex: 'total_recharged',
                    ...
                    r.id === 1 ? (
                      ...
                      <Space size={4} wrap>
                        ...
                        {r.status === '已开通' ? (
                          ...
                          <Button size="small" onClick={() => setStatus(r, '已开通')}>
                            启用
                          </Button>
                        ) : null}
                      </Space>
            ...
            <Table
              ...
              columns={[
                ...
                },
                {
                  ...
                  render: (_: any, r: any) =>
                    r.status === '待支付' ? (
                      <Button size="small" type="primary" onClick={() => confirmOrder(r)}>
                        确认到账
                      </Button>
                    ) : (
            ...
            <Table
              ...
              columns={[
                ...
                },
                {
                  ...
                  align: 'right',
                  render: (v: number) => <b style={{ color: 'var(--ui-accent)' }}>{v.toLocaleString()}</b>,
                },
                {
                  title: '启用',
                  dataIndex: 'enabled',
                  width: 70,
                  render: (v: number) => <Tag color={v ? 'green' : 'default'}>{v ? '在售' : '下架'}</Tag>,
                },
                {
                  title: '操作',
                  width: 70,
                  render: (_: any, r: any) => (
                    <button type="button" className="ui-link-button" onClick={() => openPkg(r)}>
                      编辑
                    </button>
                  ),
                },
      ...
      <Modal
        ...
        onOk={submitApprove}
        okText={approve?.status === '待审核' ? '开通' : '保存'}
...
function PlatformCreditLogs({ tenants }: { tenants: any[] }) {
  ...
  useEffect(() => {
    api.get(`/platform/credit-logs?page=${page}&size=20&${qs()}`).then(setLogs);
  }, [page, qs]);
  const doExport = async () => {
    ...
    exportCsv(
      `全平台积分流水_${new Date().toISOString().slice(0, 10)}`,
      [
        '时间',
        '企业',
        '用户',
  ...
  return (
    <Panel
      title={
        <>
          <WalletOutlined style={{ color: 'var(--ui-accent)' }} /> 全平台积分流水（跨企业 · 可筛选导出）
        </>
      }
      extra={
        <Space wrap size={6}>
