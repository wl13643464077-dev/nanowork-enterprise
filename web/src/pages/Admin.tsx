...
const FEATURE_NAME: Record<string, string> = {
  'content.image': 'AI图片生成',
  'content.video': 'AI视频生成',
  'data.export': '数据导出',
};
...
const chip = (label: string, val: React.ReactNode) => (
  <div
    key={label}
    style={{ background: 'var(--ui-surface-2)', borderRadius: 8, padding: '8px 10px', textAlign: 'center' }}
  >
          ...
          description={
            <div style={{ color: 'var(--ui-text-2)' }}>
              ...
              <div style={{ marginTop: 14 }}>
                <Button type="primary" icon={<RollbackOutlined />} onClick={() => (location.href = '/')}>
                  返回前台
                </Button>
              </div>
            </div>
...
function Overview({ onTab }: { onTab: (k: string) => void }) {
  ...
  const u = ov.usage || {};
  const aiReadiness =
    ov.readiness?.channels?.find((item: any) => item.key === 'ai') ||
    ({ effective: ov.channel === 'yunwu' ? 'configured_unverified' : 'degraded' } as any);
  const aiState = readinessMeta(aiReadiness);
  const openActive = () => {
    api.get('/admin/active-today').then(setActive);
    setDrill('active');
  };
  const openCalls = () => {
    api.get('/admin/credits/logs?size=30').then((d: any) => setLogs(d.rows || []));
    setDrill('calls');
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <Row gutter={[12, 12]}>
        <Col xs={24} lg={8}>
          <Panel title="AI通道状态 · 本企业用量" extra={<Badge status={aiState.badge} text={aiState.label} />}>
              ...
              <div style={{ background: 'var(--ui-surface-2)', borderRadius: 8, padding: '10px 12px' }}>
                ...
                <div style={{ fontSize: 12, color: 'var(--ui-muted)', marginTop: 2 }}>
                  {aiReadiness.description ||
                    (ov.channel === 'yunwu'
                      ? '已发现服务端密钥，但尚未通过最近一次显式连接测试'
                      : '未配置API Key，AI输出为内置模板，不扣积分')}
                </div>
                {aiReadiness.nextAction && (
                  <div style={{ fontSize: 11, color: 'var(--ui-muted)', marginTop: 6 }}>{aiReadiness.nextAction}</div>
                )}
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 8 }}>
                {chip('文本调用', u.textCalls)}
                {chip('生图', u.images)}
                {chip('生视频', u.videos)}
                {chip('输入tokens', (u.input ?? 0).toLocaleString())}
                {chip('输出tokens', (u.output ?? 0).toLocaleString())}
                {chip('今日调用', ov.todayCalls)}
              </div>
              <div
                style={{
                ...
                series: [
                  {
                    ...
                    type: 'bar',
                    barWidth: 16,
                    data: byDay.map((x: any) => x.credits),
                    itemStyle: { color: 'var(--ui-accent)', borderRadius: [4, 4, 0, 0] },
                  },
                ],
              }}
            />
      ...
      <RuntimeReadinessMatrix matrix={ov.readiness} />
              ...
              columns={
                [
                  ...
                  { title: '次数', dataIndex: 'n', align: 'right' },
                  {
                    ...
                    dataIndex: 'credits',
                    align: 'right',
                    render: (v: number) => <b>{(v ?? 0).toLocaleString()}</b>,
                  },
                  {
                    title: '折合人民币',
                    dataIndex: 'yuan',
          ...
          columns={
            [
              ...
              },
              {
                title: '模式',
                dataIndex: 'ai_mode',
                render: (v: string) => <Tag color={MODE_TAG[v]?.c || 'default'}>{MODE_TAG[v]?.t || v}</Tag>,
              },
            ] as any
          }
        />
...
function UsersOrg() {
  ...
  const load = () => {
    ...
    api.get('/admin/org').then(setOrg);
  };
  useEffect(() => {
    load();
    api.get('/admin/permissions').then((p: any) => setModules(p.modules || []));
  }, []);
  ...
  const openPerm = (r: any) => {
    setPermUser(r);
    setPermChecked(r.effectiveModules || []);
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            [
              {
                title: '用户名',
                dataIndex: 'username',
                render: (v: string) => <span style={{ fontFamily: MONO, fontSize: 12.5 }}>{v}</span>,
              },
              ...
              },
              {
                ...
                render: (_: any, r: any) => (
                  <Space size={10}>
                    <button type="button" className="ui-link-button" onClick={() => openEdit(r)}>
                      编辑
                    </button>
                    <button type="button" className="ui-link-button" onClick={() => openPerm(r)}>
                      菜单权限
                    </button>
                  </Space>
                ),
      ...
      {/* 菜单权限（用户级覆盖） */}
      <Modal
        title={`菜单权限 · ${permUser?.name || ''}`}
        open={!!permUser}
        onCancel={() => setPermUser(null)}
        footer={[
          <Button key="restore" danger onClick={restorePerm}>
            恢复继承角色
          </Button>,
          <Button key="cancel" onClick={() => setPermUser(null)}>
            取消
          </Button>,
          <Button key="ok" type="primary" onClick={savePerm}>
            保存覆盖
          </Button>,
        ]}
      >
        ...
        <Checkbox.Group
          value={permChecked}
          onChange={v => setPermChecked(v as string[])}
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <Panel
        title="角色 × 模块 权限矩阵"
        extra={
          <Button type="primary" size="small" onClick={saveMatrix}>
            保存矩阵
          </Button>
        }
      >
            ...
            [
              {
                ...
                render: (v: string, r: any) => (
                  <>
                    <b style={{ color: 'var(--ui-text)' }}>{v}</b>
                    <div style={{ fontSize: 11, color: 'var(--ui-muted)', fontFamily: MONO }}>{r.key}</div>
                  </>
                ),
              },
              ...meta.modules.map((m: any) => ({
                ...
                render: (_: any, r: any) => (
                  <Checkbox checked={(rm[r.key] || []).includes(m.key)} onChange={() => toggleRm(r.key, m.key)} />
                ),
              })),
            ] as any
          }
      ...
      <Panel
        title="功能级权限"
        extra={
          <Button type="primary" size="small" onClick={saveFeatures}>
            保存功能权限
          </Button>
        }
      >
        ...
        <Table
          ...
          rowKey="fkey"
          pagination={false}
          dataSource={featKeys.map(k => ({ fkey: k }))}
          columns={
            [
              {
                title: '功能',
                ...
                render: (k: string) => (
                  <>
                    <b style={{ color: 'var(--ui-text)' }}>{FEATURE_NAME[k] || k}</b>
                    <div style={{ fontSize: 11, color: 'var(--ui-muted)', fontFamily: MONO }}>{k}</div>
                  </>
                ),
              },
              ...meta.roles.map((role: any) => ({
                ...
                render: (_: any, r: any) => (
                  <Checkbox
                    checked={(ff[r.fkey] || []).includes(role.key)}
                    onChange={() => toggleFf(r.fkey, role.key)}
                  />
                ),
              })),
            ] as any
...
function MarshalsAdmin() {
  ...
  useEffect(() => {
    ...
    api
      ...
      .then((c: string[]) => setKbCats(Array.isArray(c) ? c : []))
      .catch(() => {});
    api
      .get('/sys/kb')
      .then((docs: any[]) => {
        ...
        (Array.isArray(docs) ? docs : []).forEach((d: any) => {
          if (d.enabled) stat[d.category] = (stat[d.category] || 0) + 1;
        });
        setKbStat(stat);
      })
      .catch(() => {});
  ...
  const toggleOnline = (m: any, v: boolean) =>
    api.put(`/admin/marshals/${m.id}`, { online: v }).then(() => {
      message.success(`${m.name} 已${v ? '上线' : '下线'}`);
      load();
    });
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      <Alert
        type="info"
        showIcon
        message="编辑保存为草稿，点击「同步到前台」后对新的AI调用生效（PRD §12 配置同步机制）。"
      />
      <Row gutter={[12, 12]}>
        {list.map(m => (
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <Avatar
                  ...
                  style={{ background: 'var(--ui-surface-2)', fontSize: 20, flexShrink: 0 }}
                >
                  {m.emoji}
                </Avatar>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontWeight: 600, color: 'var(--ui-text)' }}>
                    {m.name} <Badge status={m.online ? 'success' : 'default'} />
                  </div>
                  <div
                    style={{
                      fontSize: 12,
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
                {String(m.skills || '')
                  ...
                  .slice(0, 4)
                  .map((s: string) => (
                    <Tag key={s} color="blue" style={{ fontSize: 11, marginRight: 0 }}>
                      {s.trim()}
                    </Tag>
                  ))}
              </div>
              ...
              >
                <span style={{ fontSize: 11, color: 'var(--ui-muted)' }}>
                  <CloudSyncOutlined /> 最近同步：{m.synced_at || '未同步'}
                </span>
        ...
        title={
          cur ? (
            <Space>
              <Avatar
                size={24}
                src={cur.avatar || undefined}
                style={{ background: 'var(--ui-surface-2)', fontSize: 12 }}
              >
                {cur.emoji}
              </Avatar>
              编辑 · {cur.name}
        ...
        footer={
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            ...
            <Space>
              ...
              <Button type="primary" icon={<CloudSyncOutlined />} onClick={sync}>
                同步到前台
              </Button>
            </Space>
          </div>
      ...
      >
        ...
        <Form form={form} layout="vertical">
          ...
          <Form.Item name="skills" label="能力标签（逗号分隔）">
            <Input placeholder="如：短视频脚本,朋友圈文案,社群话题" />
          </Form.Item>
          <Form.Item
            label={`权威员工目录（只读 · ${(cur?.specialists || []).length}人）`}
            ...
            <div className="admin-catalog-grid">
              {(cur?.specialists || []).map((employee: any) => (
                <div className="admin-catalog-person" key={employee.employee_idx || employee.id}>
                  <b>
                    #{employee.employee_idx} · {employee.person || employee.name}
                  </b>
                  <span>
                    {employee.name} · {employee.status || '空闲'}
                  </span>
                </div>
              ))}
            </div>
          ...
          >
            <Checkbox.Group
              ...
              options={kbCats
                .filter(c => c !== '员工产出')
                .map(c => ({ label: `${c}（${kbStat[c] || 0}篇）`, value: c }))}
            />
          </Form.Item>
...
function ApiConfig() {
  ...
  const setTextPrice = (model: string, field: 'in' | 'out', v: any) =>
    setBl({ ...bl, text: { ...bl.text, [model]: { ...bl.text[model], [field]: v ?? 0 } } });
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      <Row gutter={[12, 12]}>
        ...
        <Col xs={24} lg={10}>
          <Panel
            title="A · AI通道"
            extra={<Badge status={aiState.badge} text={<span style={{ fontSize: 12 }}>{aiState.label}</span>} />}
          >
            <FieldRow label="服务商">
            ...
            <FieldRow label="API Key">
              <Input
                value={cfg.channel.key}
                readOnly
                suffix={
                  <Tag
                    color={
                      cfg.channel.keySource === 'environment'
                        ? 'green'
                        : cfg.channel.keySource === 'legacy_db'
                          ? 'orange'
                          : 'default'
                    }
                  >
            ...
            <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 10, flexWrap: 'wrap' }}>
              <Button size="small" loading={testing} onClick={testConn}>
                🔌 测试连接
              </Button>
              {testResult && (
                <Tag color={testResult.ok ? 'green' : 'red'} style={{ margin: 0 }}>
                  {testResult.ok ? `✓ 最近验证通过（${testResult.models}个可用模型）` : `✗ ${testResult.error}`}
                </Tag>
              )}
            </div>
            <div style={{ fontSize: 11, color: 'var(--ui-muted)', marginBottom: 10 }}>
              API Key 仅从服务端环境变量 <code>YUNWU_API_KEY</code> 读取，不在数据库保存新明文。
              {cfg.channel.keySource === 'legacy_db' ? ' 当前正在兼容读取旧数据库配置，请尽快迁移到环境变量。' : ''}
              Base URL 修改后请点击页面底部“保存配置”；配置存在不代表通道已连接，最近显式测试通过后才会显示验证有效。
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 8 }}>
              {chip('文本调用', usage.calls)}
              {chip('生图', usage.images)}
              {chip('生视频', usage.videos)}
              {chip('输入tokens', (usage.input ?? 0).toLocaleString())}
              {chip('输出tokens', (usage.output ?? 0).toLocaleString())}
              {chip('失败', usage.errors)}
            </div>
          </Panel>
        </Col>
          ...
          <Panel title="B · 分层模型路由">
            {[
              ...
              <FieldRow key={k} label={<>文本 · {label}</>}>
                <Input
                  value={rt.text?.[k]}
                  onChange={e => setTextModel(k, e.target.value)}
                  style={{ fontFamily: MONO, fontSize: 12.5 }}
                />
              </FieldRow>
            ))}
            <FieldRow label="图片模型">
              <Input
                value={rt.image}
                onChange={e => setRt({ ...rt, image: e.target.value })}
                style={{ fontFamily: MONO, fontSize: 12.5 }}
              />
            </FieldRow>
            <FieldRow label="视频模型清单">
              <Select
                ...
                style={{ width: '100%' }}
                onChange={(v: string[]) =>
                  setRt({ ...rt, video: v, videoDefault: v.includes(rt.videoDefault) ? rt.videoDefault : v[0] })
                }
                tokenSeparators={[',', '，']}
      ...
      >
        <Row gutter={[12, 12]} style={{ marginBottom: 14 }}>
          <Col xs={12} md={5}>
            ...
            <InputNumber
              ...
              step={0.001}
              style={{ width: '100%' }}
              onChange={v => setBl({ ...bl, creditYuan: v ?? 0.01 })}
            />
          </Col>
          <Col xs={12} md={5}>
            <div style={{ fontSize: 12, color: 'var(--ui-text-2)', marginBottom: 4 }}>利润系数</div>
            <InputNumber
              value={bl.marginMultiplier}
              min={1}
              step={0.1}
              style={{ width: '100%' }}
              onChange={v => setBl({ ...bl, marginMultiplier: v ?? 1.5 })}
            />
          </Col>
          {[
            ['text', '文本'],
        ...
        <Row gutter={[12, 12]}>
          <Col xs={24} lg={10}>
            <div style={{ fontWeight: 600, fontSize: 13, color: 'var(--ui-text)', marginBottom: 8 }}>
              文本模型价格（¥/百万token）
            </div>
            <Table
              size="small"
                ...
                [
                  {
                    ...
                    dataIndex: 'model',
                    render: (v: string) => (
                      <span style={{ fontFamily: MONO, fontSize: 12 }}>{v === 'default' ? 'default（兜底）' : v}</span>
                    ),
                  },
                  {
                    title: '输入¥',
                    ...
                    render: (_: any, r: any) => (
                      <InputNumber
                        ...
                        min={0}
                        step={0.5}
                        value={bl.text[r.model]?.in}
                        onChange={v => setTextPrice(r.model, 'in', v)}
                        style={{ width: '100%' }}
                      />
                    ),
                  },
                  {
                    ...
                    render: (_: any, r: any) => (
                      <InputNumber
                        ...
                        min={0}
                        step={0.5}
                        value={bl.text[r.model]?.out}
                        onChange={v => setTextPrice(r.model, 'out', v)}
                        style={{ width: '100%' }}
                      />
                    ),
                  },
                  ...
                  },
                  {
                    ...
                    render: (_: any, r: any) => (
                      <InputNumber
                        ...
                        min={0}
                        step={0.1}
                        value={bl.image[r.model]}
                        onChange={v => setImagePrice(r.model, v)}
                        style={{ width: '100%' }}
                      />
                    ),
                  },
                  ...
                  },
                  {
                    ...
                    render: (_: any, r: any) => (
                      <InputNumber
                        ...
                        min={0}
                        step={1}
                        value={bl.video[r.model]}
                        onChange={v => setVideoPrice(r.model, v)}
                        style={{ width: '100%' }}
                      />
                    ),
                  },
...
function CreditsAdmin() {
  ...
  const loadUsers = () =>
    api.get('/admin/credits').then((data: any) => {
      if (Array.isArray(data)) {
        setUsers(data);
        setPoolBalance(Number(data[0]?.credits || 0));
        return;
      }
      setUsers(data?.users || []);
      setPoolBalance(Number(data?.balance || 0));
    });
  ...
  const doExport = async () => {
    ...
    const rows = (d.rows || []).map((r: any) => [
      ...
      r.spend_yuan ?? '',
      r.balance_after,
      MODE_TAG[r.ai_mode]?.t || r.ai_mode,
    ]);
    exportCsv(
      `积分流水_${new Date().toISOString().slice(0, 10)}`,
      ['时间', '用户', '功能', '类型', '模型', 'tokens(入+出)', '消耗积分', '充值积分', '折合人民币', '余额', '模式'],
      rows,
    );
  ...
  const doAdjust = async () => {
    ...
    message.success(
      `企业共享积分池已${v.delta >= 0 ? '增加' : '扣减'} ${Math.abs(v.delta)} 分，当前余额 ${res.balance} 分`,
    );
    setTarget(null);
    form.resetFields();
    loadUsers();
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <Row gutter={[12, 12]}>
        <Col xs={24} lg={9}>
          <Panel
            title="企业共享积分池"
            extra={<b style={{ color: 'var(--ui-primary)' }}>{poolBalance.toLocaleString()} 分</b>}
          >
            <Table
              size="small"
              rowKey="id"
                ...
                [
                  {
                    ...
                    render: (v: string, r: any) => (
                      <>
                        <b>{v}</b>
                        <div style={{ fontSize: 11, color: 'var(--ui-muted)', fontFamily: MONO }}>{r.username}</div>
                      </>
                    ),
                  },
            ...
            extra={
              <Space wrap size={6}>
                ...
                <DatePicker.RangePicker
                  ...
                  style={{ width: 220 }}
                  onChange={(d: any) => {
                    setRange(d ? [d[0].format('YYYY-MM-DD'), d[1].format('YYYY-MM-DD')] : []);
                    setPage(1);
                  }}
                />
                <Button size="small" type="primary" ghost onClick={doExport} disabled={!logs.total}>
                  导出CSV
                </Button>
              </Space>
            }
              ...
              columns={
                [
                  ...
                  { title: '功能', dataIndex: 'feature' },
                  {
                    title: '模型',
                    dataIndex: 'model',
                    render: (v: string) => (v ? <span style={{ fontFamily: MONO, fontSize: 11.5 }}>{v}</span> : '-'),
                  },
                  { title: '类型', dataIndex: 'kind', width: 64, render: (v: string) => KIND_LABEL[v] || v || '-' },
                  {
                    ...
                    align: 'right',
                    render: (_: any, r: any) =>
                      r.input_tokens || r.output_tokens ? `${r.input_tokens}+${r.output_tokens}` : '-',
                  },
                  {
                    title: '折合人民币',
                    dataIndex: 'spend_yuan',
                    align: 'right',
      ...
      <Modal
        title={target ? `调整企业共享积分池 · 记账账号：${target.name}` : ''}
        open={!!target}
      ...
      >
        <Form form={form} layout="vertical" style={{ marginTop: 12 }}>
          <Alert
            ...
            showIcon
            style={{ marginBottom: 12 }}
            message={`当前企业共享余额：${poolBalance.toLocaleString()} 分`}
          />
          <Form.Item
            name="delta"
            label="调整额度（正数增加，负数扣减）"
            rules={[
              { required: true, message: '请输入调整额度' },
              {
                validator: (_: any, v: any) => (v === 0 ? Promise.reject(new Error('额度不能为0')) : Promise.resolve()),
              },
            ]}
          >
          ...
          <Form.Item name="note" label="备注">
            <Input placeholder="如：月度充值 / 误操作回收" />
          </Form.Item>
        </Form>
      </Modal>
    </div>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      <Row gutter={[12, 12]}>
        <Col xs={24} lg={8}>
          <Panel title="安全设置">
            {!sec ? (
              <div style={{ padding: 30, textAlign: 'center', color: 'var(--ui-muted)' }}>加载中…</div>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                <FieldRow label="Token有效期（小时）">
                  <InputNumber
                    min={1}
                    max={168}
                    value={sec.tokenHours}
                    onChange={v => setSec({ ...sec, tokenHours: v ?? 8 })}
                    style={{ width: '100%' }}
                  />
                </FieldRow>
                <FieldRow label="手机号脱敏">
                  <Switch checked={!!sec.maskPhone} onChange={v => setSec({ ...sec, maskPhone: v })} />
                  <span style={{ fontSize: 11, color: 'var(--ui-muted)', marginLeft: 8 }}>
                    员工侧客户手机号显示为 138****8888
                  </span>
                </FieldRow>
                <FieldRow label="可导出数据角色">
                  <Select
                    mode="multiple"
                    value={sec.exportRoles || []}
                ...
                <FieldRow label="登录失败锁定（次）">
                  <InputNumber
                    min={3}
                    max={20}
                    value={sec.loginFailLock}
                    onChange={v => setSec({ ...sec, loginFailLock: v ?? 5 })}
                    style={{ width: '100%' }}
                  />
                </FieldRow>
                <Button type="primary" block onClick={saveSec}>
                  保存安全设置
                </Button>
              </div>
            )}
                ...
                [
                  {
                    ...
                    width: 70,
                    render: (v: string) => (
                      <Tag color={v === 'image' ? 'blue' : 'purple'}>{v === 'image' ? '图片' : '视频'}</Tag>
                    ),
                  },
                  {
                    title: '模型',
                    dataIndex: 'model',
                    width: 130,
                    render: (v: string) => <span style={{ fontFamily: MONO, fontSize: 11.5 }}>{v || '-'}</span>,
                  },
                  { title: '提示词', dataIndex: 'prompt', ellipsis: true },
                  {
                    ...
                    width: 80,
                    render: (v: string) => (
                      <Tag color={v === '成功' ? 'green' : v === '失败' ? 'red' : 'processing'}>{v}</Tag>
                    ),
                  },
              ...
              columns={
                [
                  ...
                  },
                  {
                    ...
                    dataIndex: 'ua',
                    ellipsis: true,
                    render: (v: string) => <span style={{ fontSize: 11, color: 'var(--ui-muted)' }}>{v || '-'}</span>,
                  },
                ] as any
              }
            />
