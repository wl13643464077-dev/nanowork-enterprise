...
// 消耗锚点（用于"约可用"换算提示）
const estUse = (c: number) =>
  `≈ ${Math.floor(c / 75)} 张图 / ${Math.floor(c / 1500)} 条视频 / ${(c / 1).toLocaleString()} 次文本`;
      ...
      <Row gutter={[12, 12]}>
        <Col xs={24} md={8}>
          <StatCard
            ...
            color="var(--ui-accent)"
            label="企业积分余额"
            value={(bal.credits ?? 0).toLocaleString()}
            suffix="分"
          />
        </Col>
        <Col xs={12} md={8}>
        ...
        <Row gutter={[14, 14]}>
          {packages.map(p => (
            <Col xs={24} sm={12} lg={8} xl={6} key={p.id}>
              ...
              >
                {p.tag && (
                  <Tag color={TAG_COLOR[p.tag] || 'blue'} style={{ position: 'absolute', top: 14, right: 14 }}>
                    {p.tag}
                  </Tag>
                )}
          ...
          <Table
            ...
            columns={[
              ...
              },
              {
                ...
                width: 80,
                render: (v: string) => (
                  <Tag color={v === 'recharge' ? 'green' : 'blue'} style={{ fontSize: 11 }}>
                    {v === 'recharge'
                      ? '充值'
                      : v === 'text'
                        ? '文本'
        ...
        {pickPkg && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12, fontSize: 13 }}>
            <div style={{ background: 'var(--ui-surface-2)', borderRadius: 10, padding: '12px 14px', lineHeight: 2 }}>
              ...
              </b>（含赠送 {pickPkg.bonus_credits.toLocaleString()}）<br />
        ...
        {orderResult && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12, fontSize: 13 }}>
            <div style={{ background: 'var(--ui-surface-2)', borderRadius: 10, padding: '12px 14px', lineHeight: 2 }}>
              ...
              <div>
                到账积分：
                <b style={{ color: 'var(--ui-accent)' }}>{orderResult.package?.total_credits?.toLocaleString()}</b>
              </div>
            </div>
            <Alert type="warning" showIcon message={orderResult.payGuide} />
            <div style={{ fontSize: 12, color: 'var(--ui-muted)' }}>
              支付完成后，平台确认到账，积分将自动充入企业积分池，可在「充值记录」查看状态。
            </div>
          </div>
        )}
      </Modal>
