...
import { Row, Col, Table, Tag, Badge, Progress, Select, Input, Collapse, Timeline, Empty, message, Modal, Drawer, Spin, Button, Form, InputNumber, Alert, Tooltip, Popconfirm } from 'antd';
import {
...
const catTagColor: Record<string, string> = { '内容资产': 'blue', '知识资产': 'purple', '客户资产': 'cyan', '数据资产': 'orange', '品牌资产': 'magenta' };
const catChartColor: Record<string, string> = { '内容资产': 'var(--ui-accent)', '知识资产': '#8a7450', '客户资产': '#22c4a8', '数据资产': '#f6a02d', '品牌资产': '#f25b6b' };
const statusColor: Record<string, string> = { '使用中': '#22c4a8', '闲置': '#f6a02d', '待归档': '#70757a', '已归档': 'var(--ui-muted)' };
const actionColor: Record<string, string> = {
  '创建': 'var(--ui-accent)', '新增': 'var(--ui-accent)', '入库': 'var(--ui-accent)', '启用': '#22c4a8', '使用': '#22c4a8',
  '调用': '#70757a', '更新': '#8a7450', '闲置': '#f6a02d', '流转': '#f6a02d', '归档': 'var(--ui-muted)',
};
...
export default function Assets() {
  ...
  const openDrill = (kind: string) => {
    ...
    api.get(`/assets/drill/${kind}`).then(d => setDrill({ open: true, kind, data: d })).catch(() => {});
  };
  ...
  const openTrace = (id: number) => {
    ...
    api.get(`/assets/${id}/trace`).then(d => setTrace({ open: true, data: d })).catch(() => setTrace({ open: false, data: null }));
  };
  ...
  const openVisualDetail = (kind: string, params: Record<string, any> = {}, fallbackTitle = '明细追溯') => {
    ...
    api.get(`/assets/visual-drill/${kind}${qs ? `?${qs}` : ''}`)
  ...
  const loadList = () => {
    api.get(`/assets?category=${encodeURIComponent(category || '')}&status=${encodeURIComponent(status || '')}&kw=${encodeURIComponent(kw)}&page=${page}&size=10`).then(setList);
  };
  ...
  const submitAssetImport = async () => {
    ...
    const rows = batchRows.length ? batchRows : [{
      name: v.name,
      category: v.category,
      value: v.value,
      status: v.status,
    ...
    message.success(`导入完成：成功 ${ret.created?.length || 0} 项${ret.errors?.length ? `，失败 ${ret.errors.length} 项` : ''}`);
    setImportOpen(false);
  ...
  const warnGroups = [
    { key: 'idle', label: '闲置超90天', tag: 'orange', color: '#f6a02d', n: wCounts.idle ?? (warnings?.idle || []).length, rows: warnings?.idle || [] },
    { key: 'incomplete', label: '信息不完整', tag: 'red', color: '#f25b6b', n: wCounts.incomplete ?? (warnings?.incomplete || []).length, rows: warnings?.incomplete || [] },
    { key: 'lowUse', label: '零调用', tag: 'default', color: 'var(--ui-muted)', n: wCounts.lowUse ?? (warnings?.lowUse || []).length, rows: warnings?.lowUse || [] },
  ];
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      {/* KPI 行（逐卡可点钻取） */}
      <Row gutter={[12, 12]}>
        <Col xs={12} md={8} xl={4}><StatCard icon={<DatabaseOutlined />} color="var(--ui-accent)" label="资产总数"
          value={summary.total ?? '-'} suffix="项" onClick={() => openDrill('total')} /></Col>
        <Col xs={12} md={8} xl={4}><StatCard icon={<PayCircleOutlined />} color="#f6a02d" label="资产总价值"
          value={fmtWan(summary.totalValue || 0)} onClick={() => openDrill('value')} /></Col>
        <Col xs={12} md={8} xl={4}><StatCard icon={<PlusSquareOutlined />} color="#22c4a8" label="本月新增资产"
          value={summary.monthNew ?? '-'} suffix="项" onClick={() => openDrill('month-new')} /></Col>
        <Col xs={12} md={8} xl={4}><StatCard icon={<PlayCircleOutlined />} color="#8a7450" label="使用中资产"
          value={summary.inUse ?? '-'} suffix={summary.total ? `占比 ${((summary.inUse / summary.total) * 100).toFixed(1)}%` : ''} onClick={() => openDrill('in-use')} /></Col>
        <Col xs={12} md={8} xl={4}><StatCard icon={<FundOutlined />} color="#70757a" label="资产使用率"
          value={`${summary.useRate ?? '-'}%`} onClick={() => openDrill('use-rate')} /></Col>
        <Col xs={12} md={8} xl={4}><StatCard icon={<ThunderboltOutlined />} color="#f25b6b" label="调用次数"
          value={summary.calls ?? '-'} trend={summary.valueGrowth} trendLabel="价值增长" onClick={() => openDrill('calls')} /></Col>
      </Row>

      {/* 第二行：分类概览 + 价值趋势 + 健康度 */}
      <Row gutter={[12, 12]}>
        <Col xs={24} lg={8}>
          <Panel title="资产分类概览" extra={<span style={{ fontSize: 12, color: 'var(--ui-muted)' }}>共 {catTotal} 项</span>}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <div style={{ width: '46%', flexShrink: 0 }}>
                <Chart height={210} onClick={(p: any) => {
                  setCategory(p.name);
                  setPage(1);
                  ...
                  title: {
                    text: String(catTotal || '-'), subtext: '资产总数', left: 'center', top: '36%',
                    textStyle: { fontSize: 22, fontWeight: 700, color: 'var(--ui-text)' },
                    subtextStyle: { fontSize: 11, color: 'var(--ui-muted)' },
                  },
                  series: [{
                    type: 'pie', radius: ['58%', '80%'], avoidLabelOverlap: false, label: { show: false },
                    itemStyle: { borderColor: '#fff', borderWidth: 2, borderRadius: 4 },
                    data: categories.map((c, i) => ({
                      name: c.category, value: c.n,
              ...
              <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 9, minWidth: 0 }}>
                {categories.map((c, i) => (
                  <div key={c.category} onClick={() => { setCategory(c.category); setPage(1); openVisualDetail('category', { category: c.category }, `${c.category}资产明细`); }}
                    style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, cursor: 'pointer' }}>
                    <span style={{ width: 8, height: 8, borderRadius: 2, flexShrink: 0, background: catChartColor[c.category] || CHART_COLORS[i % CHART_COLORS.length] }} />
                    <span style={{ color: 'var(--ui-text-2)', flex: 1, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{c.category}</span>
        ...
        <Col xs={24} lg={8}>
          <Panel title="资产价值趋势" extra={<Tag color="blue" style={{ fontSize: 11 }}>近{trend.length || 0}周</Tag>}>
            <Chart height={210} onClick={(p: any) => {
              ...
              tooltip: {
                ...
                formatter: (ps: any) => {
                  ...
                  ps.forEach((p: any) => { s += `${p.marker}${p.seriesName}：${p.seriesName === '资产价值' ? fmtWan(p.value) : `${p.value} 项`}<br/>`; });
                  return s;
                },
              },
              ...
              yAxis: [
                { type: 'value', ...axisStyle, axisLabel: { color: 'var(--ui-muted)', fontSize: 11, formatter: (v: any) => (v >= 10000 ? `${(v / 10000).toFixed(0)}万` : v) } },
                { type: 'value', ...axisStyle, splitLine: { show: false } },
              ],
              series: [
                {
                  name: '资产价值', type: 'line', smooth: true, symbol: 'none', yAxisIndex: 0,
                  data: trend.map(x => x.value), lineStyle: { width: 2.5, color: 'var(--ui-accent)' }, itemStyle: { color: 'var(--ui-accent)' },
                ...
                {
                  name: '资产数量', type: 'line', smooth: true, symbol: 'circle', symbolSize: 5, yAxisIndex: 1,
                  data: trend.map(x => x.count), lineStyle: { width: 2, color: '#22c4a8' }, itemStyle: { color: '#22c4a8' },
                },
              ],
            }} />
          </Panel>
        ...
        <Col xs={24} lg={8}>
          <Panel title="资产健康度" extra={<Tag color={score >= 80 ? 'green' : score >= 60 ? 'orange' : 'red'} style={{ fontSize: 11 }}>{scoreLevel}</Tag>}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 20, padding: '8px 4px' }}>
              <Progress type="dashboard" percent={score} size={150} strokeColor={scoreColor} strokeWidth={9}
                format={() => (
                  <div>
                    <div style={{ fontSize: 30, fontWeight: 700, color: 'var(--ui-text)' }}>{health ? score : '-'}<span style={{ fontSize: 14, fontWeight: 500 }}>分</span></div>
                    <div style={{ fontSize: 12, color: scoreColor, marginTop: 2 }}>{health ? scoreLevel : ''}</div>
                  </div>
                )} />
              <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 14, minWidth: 0 }}>
                {(health?.subs || []).map((s: any, i: number) => (
                  <div key={s.name} onClick={() => openVisualDetail('health', { part: s.name }, `${s.name}明细`)}
                    style={{ cursor: 'pointer' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, marginBottom: 3 }}>
                      <span style={{ color: 'var(--ui-text-2)' }}>{s.name}</span>
                      <span style={{ color: 'var(--ui-text)', fontWeight: 600 }}>{s.value}%</span>
                    </div>
                    <Progress percent={s.value} showInfo={false} size="small" strokeColor={CHART_COLORS[i % CHART_COLORS.length]} />
                  </div>
                ))}
              </div>
            </div>
      ...
      <Row gutter={[12, 12]}>
        <Col xs={24} lg={17}>
          <Panel title="资产清单" extra={
            <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
              <Select size="small" placeholder="分类" allowClear style={{ width: 104 }} value={category}
                options={CATEGORIES.map(c => ({ value: c, label: c }))}
                onChange={v => { setCategory(v); setPage(1); }} />
              <Select size="small" placeholder="状态" allowClear style={{ width: 92 }} value={status}
            ...
            <Table size="small" rowKey="id" dataSource={list.rows || []}
              pagination={{
                current: page, pageSize: 10, total: list.total || 0, size: 'small',
                showSizeChanger: false, onChange: p => setPage(p), showTotal: t => `共 ${t} 项资产`,
              }}
              columns={[
                ...
                { title: '创建时间', dataIndex: 'created_at', width: 100, render: (v: string) => <span style={{ color: 'var(--ui-muted)', fontSize: 12 }}>{String(v || '').slice(0, 10)}</span> },
                {
                  ...
                  render: (_: any, r: any) => (
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                      ...
                      <Popconfirm title={`删除资产「${r.name}」？`} description="删除后进入回收站，老板/管理员可恢复。" okText="删除" cancelText="取消"
                        okButtonProps={{ danger: true }} onConfirm={() => deleteAsset(r)}>
        ...
        <Col xs={24} lg={7}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12, height: '100%' }}>
            <Panel title={<><FireOutlined style={{ color: '#f6a02d' }} /> 资产使用 TOP5</>} style={{ height: 'auto' }}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                {topUsed.length === 0 && <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="暂无数据" />}
                {topUsed.map((t, i) => (
                  <div key={t.id} onClick={() => openTrace(t.id)}
                    style={{ padding: '5px 0', borderBottom: i < topUsed.length - 1 ? '1px solid var(--ui-surface-2)' : 'none', cursor: 'pointer' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12.5 }}>
                      <span style={{
                        width: 18, height: 18, borderRadius: 5, flexShrink: 0, textAlign: 'center', lineHeight: '18px',
                        fontSize: 11, fontWeight: 700, color: i < 3 ? '#fff' : 'var(--ui-muted)',
                        background: i < 3 ? ['#f25b6b', '#f6a02d', '#70757a'][i] : 'var(--ui-border)',
                      }}>{i + 1}</span>
                      <span style={{ flex: 1, color: 'var(--ui-text)', fontWeight: 600, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{t.name}</span>
                      <span style={{ color: 'var(--ui-accent)', fontWeight: 600, flexShrink: 0 }}>{t.use_count}<span style={{ fontSize: 11, color: 'var(--ui-muted)', fontWeight: 400 }}>次</span></span>
                    </div>
                    <Progress percent={Math.round(((t.use_count || 0) / maxUse) * 100)} showInfo={false} size="small"
                      strokeColor={CHART_COLORS[i % CHART_COLORS.length]} style={{ margin: '2px 0 0 26px', width: 'calc(100% - 26px)' }} />
                  </div>
                ))}
              </div>
            </Panel>
            <Panel title={<><AlertOutlined style={{ color: '#f25b6b' }} /> 资产预警</>}
              extra={<Badge count={(wCounts.idle || 0) + (wCounts.incomplete || 0) + (wCounts.lowUse || 0)} size="small" />}
              style={{ flex: 1, height: 'auto' }} bodyStyle={{ paddingTop: 12 }}>
              <Collapse ghost size="small" style={{ margin: '0 -8px' }}
                items={warnGroups.map(g => ({
                  ...
                  children: g.rows.length === 0
                    ? <div style={{ fontSize: 12, color: 'var(--ui-muted)' }}>暂无预警资产</div>
                    : (
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                        {g.rows.slice(0, 6).map((r: any) => (
                          <div key={r.id} onClick={() => openTrace(r.id)}
                            style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, cursor: 'pointer' }}>
                            <span style={{ color: 'var(--ui-text-2)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', flex: 1 }}>{r.name}</span>
                            <span style={{ color: 'var(--ui-muted)', flexShrink: 0 }}>{r.category}</span>
                          </div>
                        ))}
                        {g.rows.length > 6 && <div style={{ fontSize: 11, color: 'var(--ui-muted)' }}>等 {g.rows.length} 项</div>}
                      </div>
                    ),
                }))} />
            </Panel>
      ...
      <Row gutter={[12, 12]}>
        <Col xs={24} lg={8}>
          <Panel title={<><SyncOutlined style={{ color: 'var(--ui-accent)' }} /> 资产生命周期分布</>}>
            <Chart height={220} onClick={(p: any) => {
              ...
              yAxis: {
                type: 'category', inverse: true, data: lifecycle.map(x => x.status),
                axisLine: { show: false }, axisTick: { show: false },
                axisLabel: { color: 'var(--ui-text-2)', fontSize: 12 },
              },
              series: [{
                type: 'bar', barWidth: 16,
                label: { show: true, position: 'right', color: 'var(--ui-text-2)', fontSize: 11, formatter: '{c} 项' },
                data: lifecycle.map((x, i) => ({
                  value: x.n,
                  itemStyle: { color: statusColor[x.status] || CHART_COLORS[i % CHART_COLORS.length], borderRadius: [0, 6, 6, 0] },
                })),
              }],
            }} />
          </Panel>
        ...
        <Col xs={24} lg={8}>
          <Panel title={<><PieChartOutlined style={{ color: '#8a7450' }} /> 资产价值分布</>}>
            <Chart height={220} onClick={(p: any) => {
              const row = categories[p.dataIndex];
              const categoryName = p.name || row?.category;
              if (categoryName) openVisualDetail('value-category', { category: categoryName }, `${categoryName}价值构成`);
            }} option={{
              tooltip: { trigger: 'axis', formatter: (ps: any) => `${ps[0]?.name}：${fmtWan(ps[0]?.value || 0)}` },
              grid: { left: 8, right: 16, top: 28, bottom: 8, containLabel: true },
              xAxis: { type: 'category', data: categories.map(c => c.category), ...axisStyle, axisLabel: { color: 'var(--ui-muted)', fontSize: 10.5, interval: 0 } },
              yAxis: { type: 'value', ...axisStyle, axisLabel: { color: 'var(--ui-muted)', fontSize: 11, formatter: (v: any) => (v >= 10000 ? `${(v / 10000).toFixed(0)}万` : v) } },
              series: [{
                type: 'bar', barWidth: 26,
                label: { show: true, position: 'top', fontSize: 10, color: 'var(--ui-muted)', formatter: (p: any) => fmtWan(p.value) },
                data: categories.map((c, i) => ({
                  value: c.v,
                  itemStyle: { color: catChartColor[c.category] || CHART_COLORS[i % CHART_COLORS.length], borderRadius: [6, 6, 0, 0] },
                })),
              }],
            }} />
          </Panel>
        ...
        <Col xs={24} lg={8}>
          <Panel title={<><ThunderboltOutlined style={{ color: '#f6a02d' }} /> 资产流转动态</>}
            extra={<span style={{ fontSize: 12, color: 'var(--ui-muted)' }}>最近 {flows.length} 条</span>}>
            <div style={{ maxHeight: 220, overflow: 'auto', paddingTop: 4 }}>
              {flows.length === 0 ? <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="暂无流转记录" /> : (
                <Timeline items={flows.map(f => ({
                  key: f.id,
                  color: actionColor[f.action] || 'var(--ui-accent)',
                  children: (
                    <div onClick={() => f.asset_id && openTrace(f.asset_id)} style={{ fontSize: 12, cursor: f.asset_id ? 'pointer' : 'default' }}>
                      <div style={{ marginBottom: 2 }}>
                        <Tag color={actionColor[f.action] || 'var(--ui-accent)'} style={{ fontSize: 11, lineHeight: '16px', marginRight: 6 }}>{f.action}</Tag>
                      ...
                      <div style={{ color: 'var(--ui-muted)', fontSize: 11, marginTop: 2 }}>{String(f.created_at || '').replace('T', ' ').slice(0, 16)}</div>
                    </div>
                  ),
                }))} />
              )}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {(drill.data.cats || []).length > 0 && (
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(130px,1fr))', gap: 8 }}>
                {drill.data.cats.map((c: any) => (
                  <div key={c.category} style={{ background: 'var(--ui-surface-2)', borderRadius: 8, padding: '8px 12px' }}>
            ...
            {(drill.data.sts || []).length > 0 && (
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                {drill.data.sts.map((s: any) => <Tag key={s.status} color={s.status === '使用中' ? 'green' : s.status === '闲置' ? 'orange' : 'default'} style={{ fontSize: 12, padding: '2px 10px' }}>{s.status} {s.n}项</Tag>)}
              </div>
            )}
            {(drill.data.rows || []).length > 0 && (
              <Table size="small" rowKey="id" dataSource={drill.data.rows} pagination={{ pageSize: 8, size: 'small' }}
                onRow={(r: any) => ({ onClick: () => openTrace(r.id), style: { cursor: 'pointer' } })}
                columns={[
                  { title: '资产', dataIndex: 'name', ellipsis: true, render: (v: any) => <b>{v}</b> },
                  { title: '类别', dataIndex: 'category', width: 90, render: (v: any) => <Tag style={{ margin: 0 }}>{v}</Tag> },
                  { title: '估值', dataIndex: 'value', width: 90, align: 'right', render: (v: any) => fmtWan(v || 0) },
                  ...(drill.kind === 'month-new' || drill.kind === 'total' ? [{ title: '登记时间', dataIndex: 'created_at', width: 100, render: (v: any) => (v || '').slice(5, 16) }] : []),
                  ...(drill.kind !== 'value' ? [{ title: '状态', dataIndex: 'status', width: 76, render: (v: any) => <Tag color={v === '使用中' ? 'green' : 'default'} style={{ margin: 0 }}>{v}</Tag> }] : []),
                  { title: '调用', dataIndex: 'use_count', width: 60, align: 'right', render: (v: any) => v ?? 0 },
                ]} />
            )}
            <div style={{ fontSize: 11.5, color: 'var(--ui-muted)', lineHeight: 1.8 }}>{drill.data.note} 点击任意资产行可溯源（来源原文+流转时间线）。</div>
          </div>
        )}
      </Modal>
      ...
      <Modal open={sourceDetail.open} width={900} footer={null}
        ...
        {sourceDetail.loading ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            <Table size="small" rowKey={(r: any, i?: number) => r.id || i}
              dataSource={sourceDetail.data?.rows || []}
              pagination={{ pageSize: 8, size: 'small' }}
              locale={{ emptyText: <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="当前权限范围暂无资产样本" /> }}
              onRow={(r: any) => ({ onClick: () => (r.id || r.asset_id) && openTrace(r.id || r.asset_id), style: { cursor: (r.id || r.asset_id) ? 'pointer' : 'default' } })}
              columns={(sourceDetail.data?.columns || []).map((c: any) => ({
                ...c,
                ellipsis: true,
                render: (v: any) => c.type === 'money' ? fmtMoney(v || 0) : renderSourceCell(v),
              }))} />
            <div style={{ fontSize: 11.5, color: 'var(--ui-muted)' }}>点击任意资产行可继续打开单资产溯源，查看来源对象和流转时间线。</div>
          </div>
        )}
      </Modal>

      {/* ===== 资产溯源抽屉 ===== */}
      <Drawer open={trace.open} width={520} onClose={() => setTrace({ open: false, data: null })}
        title={trace.data ? <span>🔍 资产溯源 · {trace.data.name}</span> : '资产溯源'}>
        {!trace.data ? <div style={{ textAlign: 'center', padding: 40 }}><Spin /></div> : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap', fontSize: 12.5, background: 'var(--ui-surface-2)', borderRadius: 8, padding: '10px 12px' }}>
              <span><Tag style={{ margin: 0 }}>{trace.data.category}</Tag></span>
              <span>估值 <b style={{ color: '#f6a02d' }}>{fmtWan(trace.data.value || 0)}</b></span>
              <span>状态 <Tag color={trace.data.status === '使用中' ? 'green' : 'default'} style={{ margin: 0 }}>{trace.data.status}</Tag></span>
              <span>调用 <b>{trace.data.use_count ?? 0}</b> 次</span>
              <span>登记 {(trace.data.created_at || '').slice(0, 16)}</span>
            </div>

            <Panel title={`📌 来源：${trace.data.source.type}`}>
              <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--ui-text)', marginBottom: 6 }}>{trace.data.source.label}</div>
              {trace.data.source.meta && <div style={{ fontSize: 11.5, color: 'var(--ui-muted)', marginBottom: 6 }}>{trace.data.source.meta}</div>}
              {trace.data.source.preview && (
                <div style={{ whiteSpace: 'pre-wrap', fontSize: 12, lineHeight: 1.8, maxHeight: 180, overflow: 'auto',
                  background: 'var(--ui-surface-2)', border: '1px solid var(--ui-border)', borderRadius: 8, padding: '8px 10px', color: 'var(--ui-text-2)' }}>
                  {trace.data.source.preview}{String(trace.data.source.preview).length >= 400 ? '…' : ''}
                </div>
              )}
              {trace.data.source.leadId && (
                <Button size="small" type="primary" ghost style={{ marginTop: 8 }} onClick={() => setCustId(trace.data.source.leadId)}>
                  打开客户完整档案 ›
                </Button>
              )}
            </Panel>
            ...
            <Panel title={`流转时间线（${(trace.data.flows || []).length}）`}>
              {(trace.data.flows || []).length === 0
                ...
                : <Timeline items={trace.data.flows.map((f: any) => ({
                    children: <div style={{ fontSize: 12.5 }}><b>{f.action}</b> <span style={{ color: 'var(--ui-muted)', fontSize: 11 }}>{(f.created_at || '').slice(5, 16)}</span>
                      {f.note && <div style={{ color: 'var(--ui-text-2)' }}>{f.note}</div>}</div>,
                  }))} />}
            </Panel>
      ...
      <Modal open={importOpen} title="导入数据资产" okText="确认导入" width={720}
        onOk={submitAssetImport} onCancel={() => setImportOpen(false)} destroyOnClose>
        ...
        <Form form={importForm} layout="vertical" initialValues={{ category: '数据资产', status: '使用中' }}>
          <Form.Item name="batchText" label="批量粘贴">
            <Input.TextArea rows={5} placeholder={'资产名称\t分类\t估值\t状态\t归属\t来源备注\n端午礼盒产品手册\t知识资产\t5000\t使用中\t市场部\t线下物料整理入库'} />
          </Form.Item>
          <div style={{ fontSize: 12, color: 'var(--ui-muted)', margin: '-4px 0 12px' }}>不填批量粘贴时，使用下方单条登记。</div>
          <Row gutter={10}>
            <Col span={12}><Form.Item name="name" label="资产名称"><Input placeholder="例如：企业团餐沟通手册" /></Form.Item></Col>
            <Col span={12}><Form.Item name="category" label="分类">
              <Select options={CATEGORIES.map(c => ({ value: c, label: c }))} /></Form.Item></Col>
            <Col span={8}><Form.Item name="value" label="估值"><InputNumber min={0} precision={0} style={{ width: '100%' }} prefix="¥" /></Form.Item></Col>
            <Col span={8}><Form.Item name="status" label="状态">
              <Select options={STATUSES.map(s => ({ value: s, label: s }))} /></Form.Item></Col>
            <Col span={8}><Form.Item name="owner" label="归属"><Input placeholder="部门/人员" /></Form.Item></Col>
          </Row>
