...
import { Alert, Button, Drawer, Empty, Form, Input, List, message, Select, Skeleton, Tag } from 'antd';
import {
  CalendarOutlined,
  CameraOutlined,
  EyeOutlined,
...
const TOOLS: ToolDefinition[] = [
  ...
  },
  {
    ...
    title: '视频混剪',
    short: '手机素材成片',
    description: '把门店手机视频整理为一条有开头、有卖点、有行动指令的竖屏短视频方案。',
    icon: <PlayCircleOutlined />,
    accent: '#4f77dd',
    employee: '章文案',
    employeeIdx: 140,
    cost: '免费',
    inputs: ['素材说明', '目标平台', '成片目的'],
    output: '分镜 + 口播 + 字幕 + 配乐与剪辑指令',
  },
  {
    key: 'pcal',
    title: '私域日历',
  ...
  },
  {
    ...
    title: '产品图文',
    short: '一份产品多端用',
    description: '围绕一道菜或一款套餐，形成外卖、小红书、朋友圈和到店物料共用的卖点系统。',
    icon: <CameraOutlined />,
    accent: '#2784c7',
    employee: '章文案',
    employeeIdx: 140,
    ...
    <div className="toolbox-page">
      <header className="toolbox-hero">
        <div>
          ...
          <p>
            8 个高频经营工具，把老板的输入变成可执行的结构化草案；全部免费，不消耗积分。已配置 AI
            模型时由对应数字员工真实生成，未配置时自动使用本地安全模板（结果页会如实标注执行模式）。
          </p>
        </div>
      ...
      <div className="toolbox-layout">
        <nav className="toolbox-nav" aria-label="工具列表">
          <div className="toolbox-nav-title">
            全部工具 <span>{TOOLS.length}</span>
          </div>
          {TOOLS.map(tool => (
            <button
              key={tool.key}
              className={active.key === tool.key ? 'active' : ''}
        ...
        <main className="toolbox-workbench">
          ...
          <div className="toolbox-contract">
            <div>
              ...
              <small>餐饮数字员工 #{active.employeeIdx}</small>
            </div>
            <div>
              <span>你需要准备</span>
              <strong>{active.inputs.join(' · ')}</strong>
              <small>缺失信息会在结果中标记为假设</small>
            </div>
            <div>
              <span>最终交付</span>
          ...
          <Form form={form} layout="vertical" requiredMark={false} className="toolbox-form">
            ...
            <div className="toolbox-submit-row">
              ...
              <Button type="primary" size="large" icon={<ThunderboltOutlined />} loading={running} onClick={runTool}>
                开始运行
              </Button>
            </div>
          </Form>
              ...
              renderItem={run => {
                ...
                return (
                  <List.Item onClick={() => setViewRun(run)}>
                    ...
                    <Tag
                      color={
                        run.status === 'done' || run.status === '已完成'
                          ? 'green'
                          : run.status === 'failed'
                            ? 'red'
                            : 'processing'
        ...
        {viewRun && (
          <div className="tool-result">
            <div className="tool-result-provenance">
              <div>
                <span>执行模式</span>
                <strong>{viewRun.provenance?.mode || '本地模板'}</strong>
              </div>
              <div>
                <span>提示词快照</span>
            ...
            <section>
              ...
              <div className="tool-result-body">
                <Markdown content={viewRun.resultMd || '结果正在生成，请稍后刷新。'} />
              </div>
            </section>
            <section>
              <h3>来源与证据</h3>
              {viewRun.evidence?.length ? (
                ...
                <Alert type="warning" showIcon message="本次没有联网来源，只能作为待核验的执行草案。" />
              )}
            </section>
          </div>
        )}
