...
function parseIsolatedFromDb() {
  ...
  if (!m) { console.error('❌ 无法从 server/src/db.js 解析 ISOLATED 集合，请检查该文件。'); process.exit(2); }
...
function callText(s, openIdx) {
  ...
  for (let i = openIdx; i < s.length; i++) {
    if (s[i] === '(') d++;
    else if (s[i] === ')') { d--; if (d === 0) return s.slice(openIdx, i + 1); }
  }
  ...
  for (const file of fs.readdirSync(full).filter(f => f.endsWith('.js'))) {
    ...
    while ((m = re.exec(s))) {
      ...
      for (const t of ISOLATED) {
        ...
        const mainFrom = new RegExp(`from\\s+${t}\\b`, 'i').test(call) && !new RegExp(`join\\s+${t}\\b`, 'i').test(call.replace(new RegExp(`from\\s+${t}\\b`, 'i'), ''));
        ...
        const relational = /\$\{where\}|\$\{ownerFilter\}|\$\{[A-Za-z_$][\w$]*\.where\}/.test(call)
          || /\b(partner_id|lead_id|activity_id|asset_id|marshal_id|specialist_id|task_id)\s*=/i.test(call);
        findings.push({ file, line, table: t, relational, snippet: call.replace(/\s+/g, ' ').slice(0, 90) });
        if (relational) review++; else high++;
        break;
      }
    }
  }
...
console.log(`\n=== 多租户隔离自检 ===`);
console.log(`监控 ${ISOLATED.length} 张隔离表（清单自动同步自 db.js 的 ISOLATED 集合 + 显式隔离表 ${EXTRA_EXPLICIT.join('/')}），扫描 ${DIRS.join(' / ')}`);
...
if (highs.length) {
  ...
  highs.forEach(f => console.log(`  ${f.file}:${f.line}  [${f.table}]  ${f.snippet}`));
}
if (reviews.length) {
  console.log(`\n🟡 待复核（按外键关联已隔离主表，通常安全）${reviews.length} 处：`);
  reviews.forEach(f => console.log(`  ${f.file}:${f.line}  [${f.table}]  ${f.snippet}`));
}
