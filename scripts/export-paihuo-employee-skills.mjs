...
const contentCatalog = JSON.parse(
  fs.readFileSync(path.join(root, "server/catalog/content-crew.json"), "utf8"),
);
const restaurantCatalog = JSON.parse(
  fs.readFileSync(path.join(root, "server/catalog/restaurant.json"), "utf8"),
);
const identities = new Map([
  ...contentCatalog.employees.map((employee) => [
    employee.idx,
    {
      key: employee.key,
      name: employee.name,
  ...
  ...restaurantCatalog.employees.map((employee) => [
    employee.idx,
    {
      key: employee.key,
      name: employee.name,
      group: employee.group,
      department: restaurantCatalog.name,
    },
  ]),
]);
...
if (
  rows.length !== expectedIndexes.length ||
  rows.some((row, index) => row.idx !== expectedIndexes[index])
) {
...
const profiles = rows.map((row) => {
  ...
  if (
    !allowedTextModels.has(modelText) ||
    !allowedImageModels.has(modelImage)
  ) {
  ...
  if (!Array.isArray(legacySkills) || !legacySkills.length)
    throw new Error(`员工${row.idx}没有历史技能`);
  const skills = legacySkills.map((skill, skillIndex) => {
    ...
    if (
      (skill.learned_at !== null && typeof skill.learned_at !== "number") ||
...
if (
  contentSkillCount !== 65 ||
  restaurantSkillCount !== 344 ||
  skillCount !== 409
) {
  throw new Error(
    `历史技能数量不匹配：内容${contentSkillCount}/餐饮${restaurantSkillCount}/总计${skillCount}`,
  );
}
...
const result = {
  ...
  source: {
    ...
    snapshot,
    policy:
      "Only employee_config idx 0-9 and 101-160 allowlisted skill/config fields were exported.",
  },
