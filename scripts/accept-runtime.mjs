...
const baseUrl = String(
  process.env.NANOWORK_ACCEPT_BASE_URL || "http://127.0.0.1:3107",
).replace(/\/+$/u, "");
...
if (!username || !password) {
  throw new Error(
    "运行态验收需要 NANOWORK_ACCEPT_USERNAME 与 NANOWORK_ACCEPT_PASSWORD",
  );
}
...
async function request(pathname, { token, method = "GET", body } = {}) {
  ...
  assert.ok(
    response.ok,
    `${method} ${pathname} 返回 ${response.status}: ${JSON.stringify(payload)}`,
  );
  return payload;
}
...
assert.ok(
  ["boss", "admin", "platform_super"].includes(login.user?.role),
  "运行态验收账号必须具备管理权限",
);
...
function assertGuidance(profile, label) {
  ...
  assert.ok(
    String(guidance.intro || "").includes(profile.identity.name),
    `${label} 岗位介绍没有员工名称`,
  );
  assert.ok(
    String(guidance.titlePlaceholder || "").length >= 8,
    `${label} 任务标题引导不完整`,
  );
  assert.ok(
    String(guidance.requirementPlaceholder || "").length >= 12,
    `${label} 任务要求引导不完整`,
  );
  assert.ok(guidance.materialChecklist?.length >= 2, `${label} 资料清单不完整`);
  assert.ok(
    guidance.deliverableChecklist?.length >= 2,
    `${label} 交付检查不完整`,
  );
...
const restaurantProfiles = await Promise.all(
  Array.from({ length: 60 }, (_, offset) => 101 + offset).map(async (idx) => {
    const profile = await request(`/api/employee-workbench/restaurant/${idx}`, {
      token,
    });
    assert.deepEqual(
      Object.keys(profile).sort(),
      [...requiredRestaurantKeys].sort(),
      `餐饮员工 #${idx} 岗位域不完整`,
    );
...
const contentProfiles = await Promise.all(
  Array.from({ length: 10 }, (_, idx) => idx).map(async (idx) => {
    const profile = await request(`/api/employee-workbench/content/${idx}`, {
      token,
    });
    assert.deepEqual(
      Object.keys(profile).sort(),
      [...requiredContentKeys].sort(),
      `内容员工 #${idx} 岗位域不完整`,
    );
    assert.equal(profile.identity.idx, idx);
...
assert.equal(
  new Set(allProfiles.map((item) => item.dispatch.guidance.titlePlaceholder))
    .size,
  70,
);
assert.equal(
  new Set(
    allProfiles.map((item) => item.dispatch.guidance.requirementPlaceholder),
  ).size,
  70,
);
...
assert.ok(
  automations.rules.some((rule) => rule.frequency === "daily" && rule.enabled),
  "缺少启用中的每日自动任务",
);
assert.ok(
  automations.rules.some((rule) => rule.frequency === "weekly" && rule.enabled),
  "缺少启用中的每周自动任务",
);

const outputs = await request(
  "/api/analysis/employee-outputs?page=1&pageSize=5",
  { token },
);
assert.notEqual(outputs.dataset?.kind, "演示环境运行记录");
assert.match(
  outputs.dataset?.disclaimer || "",
  /不代表营业额、利润或客户转化已经提升/u,
);
  ...
  JSON.stringify(
    {
      ...
      automations: {
        ...
        dailyEnabled: automations.rules.filter(
          (rule) => rule.frequency === "daily" && rule.enabled,
        ).length,
        weeklyEnabled: automations.rules.filter(
          (rule) => rule.frequency === "weekly" && rule.enabled,
        ).length,
      },
